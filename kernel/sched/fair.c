...
/* NOTE: Do not set use_walt_cpu_util or use_walt_task_util
 *       without CONFIG_SCHED_HISI_WALT_FOR_SCHED enabled */
...
update_stats_wait_end(struct cfs_rq *cfs_rq, struct sched_entity *se)
{
	...
	if (entity_is_task(se)) {
		task_of(se)->wait_sum += rq_clock(rq_of(cfs_rq)) - se->statistics.wait_start;
		...
		for_each_cpu_and(i, tsk_cpus_allowed(p), sched_group_cpus(sg_target)) {
			...
			if (!walt_disabled && sysctl_sched_use_walt_cpu_util &&
			    ((new_util * hisi_sd_capacity_margin(i)) >
			     (capacity_orig_of(i) * SCHED_LOAD_SCALE)))
				continue;
#endif
#endif
			/*
