...
unsigned int sysctl_sched_walt_init_task_load_pct = 50;
...
#ifdef CONFIG_SCHED_HISI_WALT_FOR_SCHED
...
#define TOP_TASK_IGNORE_LOAD \
	((u64)walt_ravg_window * TOP_TASK_IGNORE_UTIL / SCHED_LOAD_SCALE)
...
static void add_top_task(struct task_struct *p, struct rq *rq)
{
	...
	if (delta_load > TOP_TASK_IGNORE_LOAD &&
	    top_task_util(rq) * 10 > capacity_curr_of(cpu_of(rq)) * 8)
...
static void rollover_top_task(struct rq *rq, int nr_full_windows)
{
...
mark_util_change_for_rollover(struct task_struct *p, struct rq *rq)
{
	...
	if (new_window && p_is_curr_task) {
		...
		sugov_mark_util_change(cpu, WINDOW_ROLLOVER);
	}
}
...
#ifdef CONFIG_SCHED_HISI_TOP_TASK
		if (p_is_curr_task) {
			rollover_top_task(rq, nr_full_windows);
...
#ifdef CONFIG_SCHED_HISI_TOP_TASK
		/* When new window, top task load must be updated.
		 * Roll over p's load, and add curr_load to top task's
		 * curr table after curr_load caculated */
...
{
	int src_cpu = cpu_of(src_rq), new_cpu = cpu_of(dest_rq);
	unsigned long flags;
	int i;
...
__read_mostly int sysctl_sched_freq_inc_notify = 200 * 1000; /* + 200MHz */
__read_mostly int sysctl_sched_freq_dec_notify = 200 * 1000; /* - 400MHz */
