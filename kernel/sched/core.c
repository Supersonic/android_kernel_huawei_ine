...
#define EARLY_DETECTION_WAITING_TAK_DURATION 11500000
#define EARLY_DETECTION_HEAVY_TAK_DURATION 120000000
#define EARLY_DETECTION_NEW_TASK_DURATION 100000000

__read_mostly unsigned int sysctl_sched_ed_heavy_task_duration =
				EARLY_DETECTION_HEAVY_TAK_DURATION;

__read_mostly unsigned int sysctl_sched_ed_waiting_task_duration =
				EARLY_DETECTION_WAITING_TAK_DURATION;

__read_mostly unsigned int sysctl_sched_ed_new_task_duration =
				EARLY_DETECTION_NEW_TASK_DURATION;

static inline bool is_ed_task(struct task_struct *p, u64 wall)
{
	if (p->wait_sum >= sysctl_sched_ed_waiting_task_duration)
		return true;

	if (wall - p->last_wake_ts >= sysctl_sched_ed_heavy_task_duration)
		return true;

	if (is_new_task(p) && wall - p->last_wake_ts >=
				sysctl_sched_ed_new_task_duration)
		return true;
...
bool early_detection_notify(struct rq *rq, u64 wallclock)
{
	...
	list_for_each_entry(p, &rq->cfs_tasks, se.group_node) {
		...
		if (is_ed_task(p, wallclock) && schedtune_prefer_idle(p)) {
			rq->ed_task = p;
