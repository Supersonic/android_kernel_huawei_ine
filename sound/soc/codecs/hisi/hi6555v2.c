		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		/*mute audio DL&R to avoid headphone pop*/
		...
		/*unmute audio DL&R to avoid headphone pop*/
...
static void _hi6555v2_reset_asp_codec(struct snd_soc_codec *codec)
{
	...
	hi6555v2_reg_write(codec, R_RST_CTRLEN, 1 << RST_EN_CODEC_N);
	hi6555v2_reg_write(codec, R_RST_CTRLDIS, 1 << RST_DISEN_CODEC_N);
	hi6555v2_reg_write(codec, R_GATE_EN, 1 << GT_CODEC_CLK);
