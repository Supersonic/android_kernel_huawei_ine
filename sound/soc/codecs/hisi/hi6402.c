...
#include "../../../drivers/hisi/hi64xx_dsp/hi6402_hifi_config.h"

#ifdef CONFIG_SND_SOC_HICODEC_DEBUG
#include "hi6402_debug.h"
#endif


/*lint -e750 -e778 -e774 -e747 -e730 -e835 -e715*/
...
static struct hs_res_detect_func hi6402_hs_res_detect_func = {
	.hs_res_detect = NULL,
	.hs_path_enable = NULL,
...
void hi6402_set_rec_pll(struct snd_soc_codec *codec,unsigned int offset,unsigned int state)
{
	...
	expression = ((MAD_BIT == offset) && (false == state) && (m_rec_state & (1<<REC_BIT)));
	if (expression) {
				...
				struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		ret = slimbus_track_activate(SLIMBUS_DEVICE_HI6402, SLIMBUS_TRACK_AUDIO_PLAY, NULL);
		break;
	case SND_SOC_DAPM_POST_PMD:
		ret = slimbus_track_deactivate(SLIMBUS_DEVICE_HI6402, SLIMBUS_TRACK_AUDIO_PLAY, NULL);
		break;
	default :
		dev_warn(codec->dev, "%s : power mode event err : %d\n", __FUNCTION__, event);
		break;
				...
				struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		ret = slimbus_track_activate(SLIMBUS_DEVICE_HI6402, SLIMBUS_TRACK_VOICE_DOWN, slimbus_params);
		...
		*/
		...
		break;
	case SND_SOC_DAPM_POST_PMD:
		ret = slimbus_track_deactivate(SLIMBUS_DEVICE_HI6402, SLIMBUS_TRACK_VOICE_DOWN, NULL);
				...
				struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		ret = slimbus_track_activate(SLIMBUS_DEVICE_HI6402, SLIMBUS_TRACK_ECREF, NULL);
		break;
	case SND_SOC_DAPM_POST_PMD:
		ret = slimbus_track_deactivate(SLIMBUS_DEVICE_HI6402, SLIMBUS_TRACK_ECREF, NULL);
		break;
	default :
		dev_warn(codec->dev, "%s : power mode event err : %d\n", __FUNCTION__, event);
		break;
...
static void hi6402_s1s3_up_state_switch(struct snd_soc_codec *codec, struct hi6402_platform_data *priv)
{
	...
	case S1S3_UP_AUDIO_CAPTURE:
		...
		ret = slimbus_track_activate(SLIMBUS_DEVICE_HI6402, SLIMBUS_TRACK_AUDIO_CAPTURE, slimbus_params);
		break;
	case S1S3_UP_VOICEUP_16K:
		...
		hi64xx_update_bits(codec, HI6402_SC_RES_RW_REG_2, 0x11, 0x11);
		ret = slimbus_track_activate(SLIMBUS_DEVICE_HI6402, SLIMBUS_TRACK_VOICE_UP, slimbus_params);
	...
	case S1S3_UP_VOICEUP_8K:
		...
		break;
	case S1S3_UP_NULL:
		ret = slimbus_track_deactivate(SLIMBUS_DEVICE_HI6402, SLIMBUS_TRACK_AUDIO_CAPTURE, NULL);
		ret += slimbus_track_deactivate(SLIMBUS_DEVICE_HI6402, SLIMBUS_TRACK_VOICE_UP, NULL);
		/* dnlink sample rate change */
		ret += slimbus_track_deactivate(SLIMBUS_DEVICE_HI6402, SLIMBUS_TRACK_VOICE_DOWN, NULL);
		break;
	case S1_UP_NULL:
...
static void hi6402_init_chip(struct snd_soc_codec *codec)
{
	...
	hi64xx_update_bits(codec, HI6402_S1_MODULE_CLK_REG, 0x3 << HI6402_S1_PGA_IL_EN_BIT, 0x3 << HI6402_S1_PGA_IL_EN_BIT);
...
static int hi6402_resmgr_init(struct hi6402_platform_data *pd)
{
	...
	cfg.hi64xx_hs_high_resistance_enable = NULL;
...
{
	pr_warn("hi6402: pll_unlock irq received!!!\n");
...
static int hi6402_codec_probe(struct snd_soc_codec *codec)
{
	...
	ret = hi64xx_mbhc_init(codec, hi6402_pd->node, &hi6402_mbhc_reg, &hi6402_hs_res_detect_func,
	...
	ret = hi6402_hifi_config_init(codec, hi6402_pd->resmgr, hi6402_pd->irqmgr, hi6402_pd->cdc_ctrl->bus_sel);
	if (0 != ret) {
...
#ifdef CONFIG_SND_SOC_HICODEC_DEBUG
	ret = hicodec_debug_init(codec, &hi6402_dump_info);
	if (ret) {
				...
				struct snd_soc_dai *dai)
{
	...
	priv->voice_params.rate = rate;
	priv->voice_params.channels = params_channels(params);
...
static int hi6402_platform_probe(struct platform_device *pdev)
{
	...
	ret = hi64xx_irq_request_irq(priv->irqmgr, IRQ_PLL_UNLOCK, hi6402_pll_unlock_handler, "pll_unlock", priv);
	if (0 != ret) {
