...
#if 0
		/* confirm with guoxiaodong, the following part no need */
		/* if sampleback is ok, delete them */
		temp = readl(pmctrl_reg_base + PMC_PPLL3CTRL1);
		temp |= PPLL3_INT_MOD;
		temp &= ~GT_CLK_PPLL3;
		writel(temp, pmctrl_reg_base + PMC_PPLL3CTRL1);

		temp = readl(pmctrl_reg_base + PMC_PPLL3CTRL0);
		temp &= ~(0xFFF << PPLL3_FBDIV_START);
		temp |= (0x40 << PPLL3_FBDIV_START);
		temp &= ~PPLL3_BP;
		temp |= PPLL3_EN;
		writel(temp, pmctrl_reg_base + PMC_PPLL3CTRL0);

		do {
			udelay(100);
			temp = readl(pmctrl_reg_base + PMC_PPLL3CTRL0);
		} while ((temp & PPLL3_LOCK) == 0);
