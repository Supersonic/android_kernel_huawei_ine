										...
										int offset, int avail)
{
	...
	dev_info(dev, "dynamic_mem rsc: da %x, pa %x, len %x, flags %x\n",
	...
	if (rproc->domain) {
        ...
        flags = ((flags & 0x70) << 1) | (flags & (~0x70));
        dev_info(dev, "flags %x\n", flags);
#endif
		ret = iommu_map_sg(rproc->domain, rsc->da, table->sgl, sg_nents(table->sgl), flags);
		if (ret != rsc->len) {
...
unsigned long get_debug_isp_clk_freq(unsigned int type)
{
    ...
    if (!get_debug_isp_clk_enable()) {
        pr_err("[%s] Debug ISPclock Disabled\n", __func__);
        return 0;
    }
