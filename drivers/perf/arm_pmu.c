...
#ifdef CONFIG_ARM_MEMLAT_MON
static void idle_pmu_update(struct arm_pmu *armpmu)
{
	struct pmu_hw_events *hw_events = this_cpu_ptr(armpmu->hw_events);
	struct perf_event *event;
...
static int idle_pmu_notify(struct notifier_block *nb, unsigned long action,
							void *data)
{
	struct arm_pmu *armpmu = container_of(nb, struct arm_pmu, idle_nb);
	struct pmu_hw_events *hw_events = this_cpu_ptr(armpmu->hw_events);
	int enabled = bitmap_weight(hw_events->used_mask, armpmu->num_events);
...
#ifdef CONFIG_ARM_MEMLAT_MON
	cpu_pmu->idle_nb.notifier_call = idle_pmu_notify;
	idle_notifier_register(&cpu_pmu->idle_nb);
#endif
...
{
#ifdef CONFIG_ARM_MEMLAT_MON
	idle_notifier_unregister(&cpu_pmu->idle_nb);
#endif
