...
static void *packing_vaddr;
static ion_phys_addr_t packing_paddr;
...
static int ion_cpudraw_alloc_table(struct sg_table **table, int nents)
{
	...
	sg = kcalloc(nents, sizeof(struct scatterlist), GFP_KERNEL);
	if (NULL == sg) {
		ret = -ENOMEM;
		goto err_free_tb;
	}
				      ...
				      unsigned long align)
{
	...
	unsigned long offset = cpubuffer_alloc(cpudraw_heap->pool,
						size, byte_w, byte_h);
...
int ion_heap_cpudraw_buffer_zero(struct ion_buffer *buffer)
{
	struct sg_table *table = NULL;

	table = buffer->cpudraw_sg_table
		? buffer->cpudraw_sg_table : buffer->priv_virt;
...
#define CPU_DRAW_STRIDE (SZ_16K)
	...
	* if need, packing sglist count is also the same as height,
	* so total will be double
	*/
	...
	for (i = 0; i < h; i++) {
		...
		if (packing_len != 0) {
			/*fill packing memory sglist
			 * & real Cpu draw memory sglist*/
			sg_set_page(cpudraw_sg,
				pfn_to_page(PFN_DOWN(paddr)), byte_w, 0);
			sg_dma_address(cpudraw_sg) = sg_phys(cpudraw_sg);
			cpudraw_sg = sg_next(cpudraw_sg);
			sg_set_page(sg,
				pfn_to_page(PFN_DOWN(packing_paddr)),
				packing_len, 0);
