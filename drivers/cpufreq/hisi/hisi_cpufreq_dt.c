...
#ifdef CONFIG_HISI_CPU_FREQ_GET
static void __iomem *g_cpufreq_reg = NULL;
void dump_curr_freq(void)
{
	if (g_cpufreq_reg)
		pr_err("cpuscbakdata:0x%x\n", readl(g_cpufreq_reg));
}

static void reg_map_init(void)
{
	struct device_node *sctrl_np = NULL;
	struct device_node *freq_np = NULL;
	...
	freq_np = of_find_node_by_name(NULL, "cpu-cluster.0");
	if (NULL == freq_np)
		return;

	ret = of_property_read_u32(freq_np, "hisilicon,hi3xxx-xfreq-scbakdata", &scbakdata);
	of_node_put(freq_np);
	if (ret || scbakdata >= PAGE_SIZE)
		return;
	...
	sctrl_reg = of_iomap(sctrl_np, 0);
	of_node_put(sctrl_np);

	if (NULL == sctrl_reg)
		return;

	g_cpufreq_reg = sctrl_reg + scbakdata;
}
#endif
