...
void set_ddr_freq(struct dev_data *d, unsigned long new_freq)
{
	if (new_freq == d->cur_ddr_freq)
		return ;

	set_hw_vote_reg(&d->hw_vote, new_freq);

	d->cur_ddr_freq = new_freq;
}

void set_cpu_votefreq(int cpu, unsigned long new_freq)
{
...
EXPORT_SYMBOL(set_cpu_votefreq);

unsigned long get_cpu_votefreq(int cpu)
{
	struct dev_data *d = per_cpu(dev_data, cpu);
	unsigned int freq;
...
EXPORT_SYMBOL(get_cpu_votefreq);
