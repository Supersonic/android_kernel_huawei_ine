...
static DEFINE_PER_CPU(struct memlat_node *, memlat_dev_node);
...
unsigned int get_min_core_freq(int cpu, unsigned long cur_ddr_freq)
{
	struct memlat_node *node;
	struct memlat_hwmon *hw, *found = NULL;
	...
	while (map->core_mhz && map->target_freq < cur_ddr_freq)
		map++;
                                                        ...
                                                        void *data)
{
	bool cores_pwrdn;
	int cpu = get_cpu();
	struct memlat_node *d = per_cpu(memlat_dev_node, cpu);
	put_cpu();
	...
	case CPU_PM_ENTER:
			...
			if (cores_pwrdn) {
				...
				d->idle_ddr_freq = get_cpu_votefreq(cpu);
				set_cpu_votefreq(cpu, 0);
	...
	case CPU_PM_EXIT:
			...
			if (d->idle_ddr_freq != 0
				&& d->idle_ddr_freq != get_cpu_votefreq(cpu)) {
				set_cpu_votefreq(cpu, d->idle_ddr_freq);
				d->idle_ddr_freq = 0;
		...
		unsigned long val, void *data)
{
	...
	if (new_freq < d->min_core_freq) {
		...
		set_cpu_votefreq(cpu, max_target_freq);
...
void stop_monitor(struct devfreq *df)
{
	...
	/* set ddr vote freq to 0 */
	set_cpu_votefreq(cpumask_any(&hw->cpus), 0);
	spin_lock(&node->idle_notif_spinlock);
	node->idle_ddr_freq = 0;
...
int register_memlat(struct device *dev, struct memlat_hwmon *hw)
{
	...
	for_each_cpu(cpu, &hw->cpus) {
		per_cpu(memlat_dev_node, cpu) = node;
	}
