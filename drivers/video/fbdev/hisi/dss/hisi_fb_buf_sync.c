...
int hisifb_buf_sync_create_fence(struct hisi_fb_data_type *hisifd, dss_overlay_t *pov_req)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,9,0)
	struct sync_file *release_fence = NULL;
	struct sync_file *retire_fence = NULL;
#else
	...
	static uint32_t s_create_fence_count = 0;
	...
	if (release_fence_fd < 0) {
		...
		s_create_fence_count++;
		if (s_create_fence_count >= DSM_CREATE_FENCE_FAIL_EXPIRE_COUNT) {
			if (lcd_dclient && !dsm_client_ocuppy(lcd_dclient)) {
				dsm_client_record(lcd_dclient, "get_unused fd failed, create fence fail!\n");
				dsm_client_notify(lcd_dclient, DSM_LCD_CREATE_FENCE_FAIL_NO);
				s_create_fence_count = 0;
			}
		}
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,9,0)
	release_fence_pt = hisifb_sync_pt_create_internal(hisifd->buf_sync_ctrl.timeline,sizeof(struct sync_pt) + 4, release_fence_val);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,9,0)
	release_fence = sync_file_create(&release_fence_pt->base);
	if (release_fence == NULL) {
		fence_put(&release_fence_pt->base);
		put_unused_fd((unsigned int)release_fence_fd);
		return -ENOMEM;
	}
	fd_install(release_fence_fd, release_fence->file);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,9,0)
	retire_fence_pt = hisifb_sync_pt_create_internal(hisifd->buf_sync_ctrl.timeline,sizeof(struct sync_pt) + 4, retire_fence_val);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,9,0)
	retire_fence = sync_file_create(&retire_fence_pt->base);
	if (retire_fence == NULL) {
		fence_put(&retire_fence_pt->base);
		put_unused_fd((unsigned int)retire_fence_fd);
		return -ENOMEM;
	}
	fd_install(retire_fence_fd, retire_fence->file);
#else
...
int hisifb_buf_sync_wait(int fence_fd)
{
	...
	ret = fence_wait_timeout(fence, true, BUF_SYNC_TIMEOUT_MSEC);
	if (ret < 0) {
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,9,0)
		hisifb_sync_timeline_signal_internal(hisifd->buf_sync_ctrl.timeline, val);
#else
...
void hisifb_buf_sync_register(struct platform_device *pdev)
{
	...
	hisifd->buf_sync_ctrl.timeline =
		hisifb_sync_timeline_create_internal(tmp_name);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,9,0)
		hisifb_sync_timeline_put_internal((struct sync_timeline *)hisifd->buf_sync_ctrl.timeline);
#else
