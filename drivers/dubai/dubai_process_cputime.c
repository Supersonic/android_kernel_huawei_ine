...
static int dubai_clear_and_update(void)
{
	...
	entry = dubai_create_log_entry(count, PROCESS_STATE_DEAD);
	if (entry == NULL) {
...
static int dubai_send_active_process(void)
{
	...
	log_entry = dubai_create_log_entry(max, PROCESS_STATE_ACTIVE);
	if (log_entry == NULL) {
 ...
 */
int dubai_get_proc_cputime(struct polling_event *event)
{
	int ret = 0;
