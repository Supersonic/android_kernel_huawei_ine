...
#ifdef CONFIG_NVE_CHECK_KIRIN980
	if (nve_ramdisk->header.valid_items > NV_ITEMS_MAX_NUMBER) {
		pr_err("[NVE][%s] valid_items error\n", __func__);
		nve_ramdisk->header.valid_items = NV_ITEMS_MAX_NUMBER;
	}
#endif
...
int nve_update_and_check_item(unsigned int update_items, unsigned int valid_check_items, struct NVE_partittion *nve_store_ramdisk, struct NVE_partittion *nve_update_ramdisk)
{
	...
	if (update_items > NV_ITEMS_MAX_NUMBER) {
		pr_err("[NVE][%s] update_items error\n", __func__);
		update_items = NV_ITEMS_MAX_NUMBER;
	}
#endif
...
static int nve_print_partition_test(struct NVE_partittion *nve_partition)
{
	...
	if (nve_partiiton_header->valid_items > NV_ITEMS_MAX_NUMBER) {
		pr_err("[NVE][%s] valid_items error\n", __func__);
		nve_partiiton_header->valid_items = NV_ITEMS_MAX_NUMBER;
	}
#endif
