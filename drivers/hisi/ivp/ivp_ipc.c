...
#define MAX_FD_NUM          (10)   //come from ppalgo
...
static int ivp_trans_sharefd_to_phyaddr(unsigned int* buff, size_t size)
{
    int ret;
    unsigned int i;
    ...
    buffer size, buff size = one cmd + one fdnum + fdnum*shard_fd + ..*/
    if ((fd_num > MAX_FD_NUM)||(0 == fd_num)||((fd_num + 2) > size))
    {
        ivp_err("ion buff number maybe wrong, num=%d, size=%zu\n", fd_num, size);
    ...
    for (i = 0; i < fd_num; i++)
    {
        ...
        if (IS_ERR(ivp_ion_fd_handle)){
            ivp_err("ion_import_dma_buf failed!\n");
                                    ...
                                    loff_t *off)
{
    ...
    if (is_ivp_in_secmode()) {
        //the third char is ipc cmd
        if (CMD_ALGORUN == *(tmp_buff + 3)) {
            ret = ivp_trans_sharefd_to_phyaddr((unsigned int *)tmp_buff, size/sizeof(unsigned int));
            if (ret < 0) {
