...
static int ivp_check_image(const struct firmware* fw) {
    ...
    memcpy(&mheader,fw->data,sizeof(mheader)); /* unsafe_function_ignore: memcpy */
...
static int ivp_get_section_info(const struct firmware* fw,struct image_section_header *psect_header, unsigned int index)
{
    int offset;
    ...
    memcpy(psect_header,fw->data+offset,sizeof(struct image_section_header)); /* unsafe_function_ignore: memcpy */
...
static int ivp_load_section(const struct firmware* fw,struct image_section_header image_sect)
{
    ...
    case IMAGE_SECTION_TYPE_DATA: {
        ...
        if(image_sect.index >= IVP_IMAGE_DDR_DEFAULT_INDEX) {
            memcpy(mem_addr,source,image_sect.size); /* unsafe_function_ignore: memcpy */
        } else {
...
static int ivp_load_firmware(const char* filename,const struct firmware* fw)
{
    ...
    struct file_header mheader;
    memcpy(&mheader,fw->data,sizeof(mheader)); /* unsafe_function_ignore: memcpy */
    ivp_info("Begin to loading image, section counts 0x%x...", mheader.sect_count);
    for (i = 0; i<mheader.sect_count; i++) {
        struct image_section_header sect;
        if(ivp_get_section_info(fw, &sect, i)) {
...
static long ivp_ioctl(struct file *fd, unsigned int cmd, unsigned long args)
{
    ...
    case IVP_IOCTL_LOAD_FIRMWARE: {
        ...
        memset((char *)&info, 0, sizeof(info));/* unsafe_function_ignore: memset  */
        if (0 != copy_from_user(&info, (void *)args, sizeof(info))) {
            ivp_err("Invalid input param size.");
...
static inline int ivp_setup_onchipmem_sections(struct platform_device *plat_devp, struct ivp_device *ivp_devp)
{
    ...
    memcpy(ivp_devp->sec_sects, ivp_devp->sects, sects_size);/* unsafe_function_ignore: memcpy */
    /*lint -restore*/
    return 0;
