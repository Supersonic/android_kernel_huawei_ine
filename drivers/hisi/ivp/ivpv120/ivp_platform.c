...
static int ivp_get_memory_section(struct platform_device *pdev, struct ivp_device *ivp_devp)
{
    int i = 0;
    int ret = -1;
    unsigned int size = 0;
...
int ivp_poweron_pri(struct ivp_device *ivp_devp)
{
    ...
    ivp_reg_write(IVP_REG_OFF_MEM_CTRL3, 0x02600260);
    ivp_reg_write(IVP_REG_OFF_MEM_CTRL4, 0x00000260);
    ivp_reg_write(IVP_REG_OFF_MST_MID_CFG, 0x00006A69);
                                          ...
                                          struct ivp_device *ivp_devp)
{
    ...
    ivp_fake_regulator = devm_regulator_get(&pdev->dev, IVP_MEDIA_REGULATOR);
    if (IS_ERR(ivp_fake_regulator)) {
        ret = -ENODEV;
        ivp_err("Get ivp fake regulator failed, ret:%d", ret);
        return ret;
