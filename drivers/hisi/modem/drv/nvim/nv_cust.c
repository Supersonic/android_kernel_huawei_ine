...
u32 nv_cust_get_head_info(const s8 * file_name, nv_dload_head *dload_head)
{
    ...
    {
        nv_record("Head_info: read %s file dload head failed!ret=0x%x.\n", file_name, ret);
...
u32 nv_cust_get_tail_info(const s8 * file_name, nv_dload_tail *dload_tail)
{
    ...
    {
        nv_record("R_Tail_info: read %s file dload tail failed!ret=0x%x\n", file_name, ret);
        return BSP_ERR_NV_READ_FILE_FAIL;
    }
...
u32 nv_cust_set_tail_info(const s8 * file_name, nv_dload_tail *dload_tail)
{
    ...
    if(ret != sizeof(nv_dload_tail))
    {
        nv_record("W_Tail_info: write %s file dload tail failed!ret=0x%x\n", file_name, ret);
        return BSP_ERR_NV_READ_FILE_FAIL;
    }
...
bool nv_cust_get_upgrade_flag(const s8 *file_name)
{
    ...
    if(ret)
    {
        nv_record("Get %s file tail info failed!\n", file_name);
        return false;
    }
...
u32 nv_cust_set_upgrade_flag(const s8 *file_name, bool flag)
{
    ...
    if(ret)
    {
       nv_record("write %s file tail info failed!\n", file_name);
       return NV_ERROR;
    }
...
u32 nv_cust_analyze(xnv_sec *sec_file)
{
    ...
    for(i=0; i<sec_file->nv_num; i++)
    {
        ...
        /*修改resume属性*/
        if((src_item->resum == NV_NOT_RESUME) && (item->has_resume == NV_HAS_RESUME) && (item->resume_value == NV_IS_RESUME))
        {
            src_item->resum = NV_IS_RESUME;
            nv_record("nvid=%d, resume = %d, this nv resume has been modified!\n", src_item->itemid, src_item->resum);
        }
...
u32 nv_cust_sec_handle(FILE *fp, nv_cust_file_ctrl *file_ctrl, nv_cust_pmap_sec_ctrl *sec_ctrl)
{
    ...
    for(i=0; i<sec_cnt; i++)
    {
        ...
        {
            nv_record("Customize: decompress failed, src_len=0x%x, ret=0x%x.!\n", cur_sec->src_length, ret);
            return ret;
        }
...
u32 nv_upgrade_modemnv(u32 prdt_id)
{
    ...
    if(ret)
    {
        nv_record("Customize Modem:Init global failed!\n");
        goto out;
    }
    ...
    if(ret != sizeof(nv_dload_head))
    {
        nv_record("Customize Modem: read dload head failed!\n");
    ...
    (void)memcpy_s((s8 *)&g_nv_bcust_ctrl.xnv_ctrl.mmap, sizeof(g_nv_bcust_ctrl.xnv_ctrl.mmap),
                                        (s8 *)&dload_head->xnv_map, sizeof(nv_file_map_s));
    (void)memcpy_s((s8 *)&g_nv_bcust_ctrl.xnv_ctrl.mbin, sizeof(g_nv_bcust_ctrl.xnv_ctrl.mbin),
                                            (s8 *)&dload_head->xnv, sizeof(nv_file_map_s));
...
u32 nv_upgrade_custnv(u32 prdt_id)
{
    ...
    if(ret)
    {
        nv_record("Customize Cust:Init global failed!\n");
        goto out;
    }
    ...
    if(ret != sizeof(nv_dload_head))
    {
        nv_record("Customize Cust: read dload head failed!\n");
        ret = BSP_ERR_NV_READ_FILE_FAIL;
        goto out;
    }
    ...
    /* cust file customize */
    g_nv_bcust_ctrl.product_id = prdt_id;
    (void)memcpy_s((s8 *)&g_nv_bcust_ctrl.cust_ctrl.mmap, sizeof(g_nv_bcust_ctrl.xnv_ctrl.mmap),
                                            (s8 *)&dload_head->cust_map, sizeof(nv_file_map_s));
    (void)memcpy_s((s8 *)&g_nv_bcust_ctrl.cust_ctrl.mbin, sizeof(g_nv_bcust_ctrl.cust_ctrl.mbin),
                                                (s8 *)&dload_head->cust, sizeof(nv_file_map_s));
...
u32 nv_upgrade_mcarrier_customize(const s8 * mcar_path)
{
    ...
    {
        ret = NV_ERROR;
        (void)memcpy_s((s8 *)&g_nv_bcust_ctrl.cust_ctrl.mmap, sizeof(g_nv_bcust_ctrl.cust_ctrl.mmap),
                                                (s8 *)&dload_head->cust_map, sizeof(nv_file_map_s));
        (void)memcpy_s((s8 *)&g_nv_bcust_ctrl.cust_ctrl.mbin, sizeof(g_nv_bcust_ctrl.cust_ctrl.mbin),
                                                    (s8 *)&dload_head->cust, sizeof(nv_file_map_s));
