...
static int sim_read_pmu_dts(struct hisi_sim_hotplug_info *info, struct device_node *np)
{
    ...
    if (ret < 0)
    {
        LOGI("read pmu_ldo12_pull_down error,ldo12 pull down default.\n");
    ...
    LOGI("%s: pmu_ldo12_pull_down: 0x%02X",__func__, info->pmu_ldo12_pull_down);
...
static void hisi_sim_det_msg_to_ccore(struct hisi_sim_hotplug_info *info, u8 sim_pluged)
{
    ...
    if (runmode_is_factory() && 0 == info->factory_send_msg_to_cp)
    {
        LOGE("%s: runmode is factory and factory_send_msg_to_cp is 0, so return.\n", __func__);
        return;
    }
...
static void inquiry_sim_det_irq_reg(struct work_struct *work)
{
    ...
    if (1 == info->send_msg_to_cp)
    {
        if (0 == info->det_debounce_checking && NULL != info->sim_debounce_delay_wq)
        {
            ...
            LOGI("%s: sim%d, det level %d debounce check begin.\n", __func__, info->sim_id, det_gpio_level);
        }
        else
        {
...
static int sim_hotplug_dts_read(struct hisi_sim_hotplug_info *info, struct device_node *np)
{
    ...
    LOGI("%s: sim%d, func_sel_state: %s, det_gpio_number: %d, send_msg_to_cp: %d, det_normal_direction: %s, card_tray_style: %s\n",
            __func__, sim_id, func_sel_state_to_string(info->func_sel_state), info->det_gpio, send_msg_to_cp,
            direction_to_string(info->det_normal_direction), card_tray_style_to_string(card_tray_style));

    LOGI("%s: sim%d, allocate_gpio: %d, shared_det_irq: %d, mux_sdsim: %d, send_card_out_msg: %d, factory_send_msg_to_cp: %d, det_debounce_wait_time, high: %d, low: %d\n",
            __func__, sim_id, allocate_gpio, shared_det_irq, mux_sdsim, send_card_out_msg, factory_send_msg_to_cp,
...
static u8 get_card1_status(struct hisi_sim_hotplug_info *info)
{
    ...
    if (!info)
    {
        LOGE("%s: info is NULL, as STATUS_NO_CARD return\n", __func__);
        return status;
    }

    if (0 == info->mux_sdsim)
    {
        LOGI("%s: sim1 mux_sdsim is 0, as STATUS_SIM return\n", __func__);
        return STATUS_SIM;
    }
...
s32 handle_msg_from_sci(u32 channel_id, u32 len, void *context)
{
    ...
    if ((u32)read_len != len)
    {
        LOGE("%s: readed len(%d) != expected len(%d)\n", __func__, read_len, len);
        return -1;
    }
    ...
    switch (request_id)
    {
        case REQUEST_CARD_STATUS:
            ...
            status = get_card1_status(context);

            LOGI("%s: bsp_icc_send status to cp, status: %d(%s)\n", __func__, status, card_status_to_string(status));
            ret = bsp_icc_send(ICC_CPU_MODEM, channel_id, &status, sizeof(status));
            if (ret != sizeof(status))
            {
                LOGE("%s: in REQUEST_CARD_STATUS, bsp_icc_send failed.\n", __func__);
            }
            break;
...
static int hisi_sim_hotplug_probe(struct spmi_device *pdev)
{
    ...
    if (ret < 0)
    {
        LOGE("%s: sim_hotplug_dts_read failed, ret: %d\n", __func__, ret);
        goto free_info;
    }
    ...
    if (ret < 0)
    {
        LOGE("%s: sim_pmu_hpd_init failed, ret: %d\n", __func__, ret);
        goto free_info;
    }
    ...
    if (ret < 0)
    {
        LOGE("%s: sim_hotplug_init failed, ret: %d\n", __func__, ret);
        goto free_info;
    }
    ...
    if (ret < 0)
    {
        LOGE("%s: sim_state_init failed, ret: %d\n", __func__, ret);
    ...
    if (ret < 0)
    {
        LOGE("%s: sim_hpd_init failed, ret: %d\n", __func__, ret);
        goto free_sim_lock;
    }
    ...
    else if (SIM1 == info->sim_id)
    {
        ...
        if (ret != 0)
        {
            LOGE("%s: bsp_icc_event_register failed, ret: %d\n", __func__, ret);
            goto free_sim_lock;
        }
    }
    ...
    if (ret < 0)
    {
        LOGE("%s: request_threaded_irq failed, ret: %d\n", __func__, ret);
        goto free_sim_lock;
    }
