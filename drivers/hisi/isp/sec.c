...
static int atfisp_loadfw(void)
{
    ...
    pr_info("[%s] + %s.(0x%lx, 0x%x), init.%x, ispmem_reserved.%x\n", __func__, loadinfo.partion_name, loadinfo.image_addr, loadinfo.image_size, dev->ispops->refs_fw, dev->ispmem_reserved);
    ...
    pr_info("[%s] - bsp_load_image.%d, %s.(0x%lx, 0x%x), init.%x, ispmem_reserved.%x\n", __func__, ret, loadinfo.partion_name, loadinfo.image_addr, loadinfo.image_size, dev->ispops->refs_fw, dev->ispmem_reserved);
	return 0;
}
...
/* alloc sec boot mem */
    ...
    if (IS_ERR(hisi_secmem_ion->boot_ion_handle)){
        if (!dsm_client_ocuppy(client_isp)) {
            ...
            dsm_client_notify(client_isp, DSM_EXTISP_LOAD_FW_ERROR_NO);
