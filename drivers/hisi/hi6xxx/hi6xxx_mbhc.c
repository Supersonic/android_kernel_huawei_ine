...
extern void hi6555v2_ibias_hsmicbias_enable(struct snd_soc_codec *codec, bool enable);
...
static inline void hi6xxx_irqs_mask_set(struct hi6xxx_mbhc_priv *priv, unsigned int irqs)
{
	...
	hi6555v2_set_reg_bits(ANA_IRQM_REG0_REG, irqs);
...
static inline void hi6xxx_irqs_mask_clr(struct hi6xxx_mbhc_priv *priv, unsigned int irqs)
{
	...
	hi6555v2_clr_reg_bits(ANA_IRQM_REG0_REG, irqs);
...
static void hi6xxx_hs_micbias_power(struct hi6xxx_mbhc_priv *priv, bool enable)
{
	...
	if (enable) {
		/*open ibias*/
		hi6555v2_ibias_hsmicbias_enable(priv->codec, true);

		/* disable ECO--key detect mode switch to NORMAL mode */
		hi6555v2_clr_reg_bits(CODEC_ANA_RW08_REG, 0x1 << CODEC_MBHD_ECO_EN_OFFSET);

		/* micbias discharge off */
		hi6555v2_clr_reg_bits(CODEC_ANA_RW07_REG, 0x1 << CODEC_HSMICB_DSCHG_OFFSET);

		/* hs micbias pu */
		hi6555v2_clr_reg_bits(CODEC_ANA_RW07_REG, 0x1 << CODEC_HSMIC_PD_OFFSET);
		msleep(10);

		/* enable NORMAL key detect and identify:1.open normal compare 2.key identify adc voltg buffer on */
		hi6555v2_clr_reg_bits(CODEC_ANA_RW08_REG, 0x1 << CODEC_MBHD_COMP_PD_OFFSET);
		hi6555v2_clr_reg_bits(CODEC_ANA_RW08_REG, 0x1 << CODEC_MBHD_BUFF_PD_OFFSET);
		...
		hi6555v2_set_reg_bits(CODEC_ANA_RW08_REG, 0x1 << CODEC_MBHD_ECO_EN_OFFSET);
		msleep(20);

		/*close ibias*/
		hi6555v2_ibias_hsmicbias_enable(priv->codec, false);
...
static void hi6xxx_hs_micbias_enable(struct hi6xxx_mbhc_priv *priv, bool enable)
{
	...
	if (enable) {
		...
		if (((snd_soc_read(priv->codec, CODEC_ANA_RW07_REG))&(0x1 << CODEC_HSMIC_PD_OFFSET)))
...
static void _hs_plug_out_detect(struct hi6xxx_mbhc_priv *priv)
{
	...
	/* mbhc vref pd */
	hi6555v2_set_reg_bits(CODEC_ANA_RW33_REG, 0x1 << CODEC_MBHD_VREF_CTRL_7_OFFSET);
...
static void _hs_plug_in_detect(struct hi6xxx_mbhc_priv *priv)
{
	...
	hi6555v2_clr_reg_bits(CODEC_ANA_RW33_REG, 0x1 << CODEC_MBHD_VREF_CTRL_7_OFFSET);
...
static void hi6xxx_mbhc_reg_init(struct snd_soc_codec *codec)
{
	...
	snd_soc_write(codec, CODEC_ANA_RW37_REG, 0x1A);
...
static void hi6xxx_mbhc_config_set(struct device_node *np, struct hi6xxx_mbhc_config *mbhc_config)
{
	...
	logi("btn_voltage threshold: btn_play=%d-%d, btn_volume_down=%d-%d, btn_volume_up=%d-%d\n",
