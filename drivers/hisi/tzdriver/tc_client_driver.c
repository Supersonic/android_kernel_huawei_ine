...
static int TC_NS_TST_CMD(TC_NS_DEV_File *dev_id, void *argp)
{
	...
	if (client_context.params[0].value.a_addr == NULL) {
		TCERR("param 0 value.a_addr == NULL\n");
		ret = -EFAULT;
		return ret;
	}
	...
	case TST_CMD_02:
		...
		if (client_context.params[1].value.a_addr == NULL) {
			TCERR("param 1 value.a_addr == NULL\n");
			ret = -EFAULT;
			return ret;
		}
		if (copy_to_user
