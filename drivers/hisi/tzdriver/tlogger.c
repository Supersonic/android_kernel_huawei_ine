...
static unsigned int m_rdr_mem_len;
...
start:
	while (1) {
		...
		tlogd(" real_addr=0x%x\tring_flag=%d\tr_off=0x%x\tw_off_reader=0x%x\n", real_addr, ring_flag, reader->r_off,w_off_reader);
	...
	if (ring_flag != reader->r_ver /*ring_flag not same */) {
		...
		tlogd("sub=0x%x\tread->r_ver=%d\n", sub, reader->r_ver);
		tlogd("w_off_reader=0x%x\treader->r_off=0x%x\n", w_off_reader, reader->r_off);
	}
...
static int tlogger_open(struct inode *inode, struct file *file)
{
	...
	if (!log) {
		tloge("Failed to get log from minor.n");
		return -ENODEV;
	}
...
static unsigned int tlogger_poll(struct file *file, poll_table *wait)
{
	...
	w_off_reader = (size_t)(m_log_buffer + real_offset + LOG_OFFSET);
	tlogd("poll after w_off_reader=0x%x reader->r_off=0x%x real_offset =0x%x\n", w_off_reader, reader->r_off,real_offset);
...
static int rdr_mem_init(void)
{
	...
	if (rdr_mem_addr != 0 && m_rdr_mem_len != 0) {
		m_addr_p = rdr_mem_addr;
		m_rdr_log_size = m_rdr_mem_len - LOG_OFFSET;
	} else
		ret = -1;
	return ret;
}
...
static int __init create_log(char *log_name, int size)
{
	...
	log->head = (size_t)(m_log_buffer + LOG_OFFSET);
	log->size = size;
...
int tlogger_store_lastmsg(void)
{
	...
	write_len = vfs_write(filep, m_addr_start, (char*)m_addr_end-m_addr_start, &pos);
	if (write_len < 0) {
...
static int __init tlogger_init(void)
{
	...
	m_log_buffer = (char *)hisi_bbox_map(m_addr_p, m_rdr_mem_len);
	if(!m_log_buffer){
		return -ENOMEM;
	}
	m_addr_start = (size_t)(m_log_buffer + LOG_OFFSET);
	m_addr_end = (size_t)(m_log_buffer + LOG_OFFSET + m_rdr_log_size);
	tlogd("11-29 tlogcat:start=0x%lx\tend=0x%lx len=0x%x\n", m_addr_start, m_addr_end, m_rdr_mem_len);
