...
int soundtrigger_socdsp_pcm_elapsed(unsigned int start, int buffer_len)
{
	...
	if (g_socdsp_pcm.fast_buffer == NULL || g_socdsp_pcm.elapsed_buffer == NULL)
		return -ENOMEM;
...
int soundtrigger_socdsp_pcm_init(void)
{
	...
	g_socdsp_pcm.elapsed_buffer = ioremap_wc(PCM_DMA_BUF_1_CAPTURE_BASE, PCM_DMA_BUF_1_CAPTURE_LEN);
