	...
	if (!recursion && ret == 0) {
		...
		if (!recursion) {
			if (rdr_rm_dir(fullname) < 0)
				BB_PRINT_DBG("%s(): failed to del %s.\n", __func__, fullname);
		}
		return -1;
	}
...
bool rdr_check_log_rights(void)
{
	...
	bool ret = true;
	int ret_s;
	u32 rdr_max_logs, rdr_max_size, rdr_log_nums = 0;
	...
	list_for_each_safe(cur, next, &__rdr_logpath_info_list) {
		...
		if ((tmpsize + size > rdr_get_logsize()) || (++rdr_log_nums > rdr_max_logs)) {
			BB_PRINT_DBG("over size: cur[0x%x], next[0x%x],max[0x%llx], or over nums:cur[%u], max[%u]\n",
