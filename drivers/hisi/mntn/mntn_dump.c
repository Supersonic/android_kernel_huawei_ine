...
static int get_mntn_dump_reserve_addr(void)
{
	...
	if (name && strncmp(name, "okay", sizeof("okay")) != 0 && strncmp(name, "ok", sizeof("ok")) != 0){
		pr_err("%s: get status(%.7s)  error\n", __func__, name);
		return -1;
	}

	p = (unsigned long *)of_get_property(np, "reg", NULL);
	if (!p) {
		pr_err("%s: get reg fail, len =%d\n", __func__, len);
		goto err;
	}
	...
	if (!g_mntn_dump_reserved_addr || !g_mntn_dump_size) {
		pr_err("get_mntn_dump_addr Error: addr: 0x%lx, size:0x%x\n",
			g_mntn_dump_reserved_addr, g_mntn_dump_size);
		goto err;
	}

	pr_err("get_mntn_dump_addr addr 0x%lx, size:0x%x\n",
		g_mntn_dump_reserved_addr, g_mntn_dump_size);
	of_node_put(np);
	return 0;
err:
...
int mntn_dump_init(void)
{
	...
	pr_err("%s: mntn dump base addr:%pK\n", __func__, g_mntn_dump_base);
	...
	for (i = 0; i < MNTN_DUMP_MAX; i++) {
		...
		if (offset >= MNTN_DUMP_MAXSIZE) {
			pr_err("%s: mntn dump size is out of range\n", __func__);
			goto err;
		}
	}
...
int register_mntn_dump(int mod_id, unsigned int size, void **vaddr)
{
	...
	if (!vaddr) {
		pr_err("%s: module id[%d], vaddr is NULL\n", __func__, mod_id);
		return -1;
	}
	...
	*ptr = 0;
	...
	if (mod_id < MNTN_DUMP_HEAD || mod_id >= MNTN_DUMP_MAX) {
		pr_err("%s: module id[%d] is invalid\n", __func__, mod_id);
		return -1;
	}

	if (!g_mntn_dump_init) {
		if (mntn_dump_init()) {
			pr_err("%s: module id[%d] fail\n", __func__, mod_id);
			return -1;
		}
	}
	raw_spin_lock(&g_mdump_lock);
	if (g_mdump_mem_info[mod_id].size < size) {
		pr_err("%s: module[%d] size(0x%x) is invalid\n", __func__, mod_id, size);
		goto err;
	}

	i = g_mdump_head->nums;
	if (i >= MNTN_DUMP_MAX) {
		pr_err("%s: mntn dump data corruption(nums: %d)\n", __func__, i);
		goto err;
	}
