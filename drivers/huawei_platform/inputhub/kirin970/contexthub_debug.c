...
#ifdef CONFIG_HUAWEI_DUBAI
static int iomcu_dubai_log_process(const pkt_header_t *head)
{
	uint32_t *data;

	if(!head)
		return -1;
	pkt_dubai_log_report_t* dubai_t = (pkt_dubai_log_report_t *)head;
	data = (uint32_t *)&dubai_t[1];
	switch(dubai_t->event_type){
		case DUBAI_EVENT_TEST:
			hwlog_info("DUBAI_EVENT_TEST: %d, %d , %d\n",data[0],data[1],data[2]);
			break;
		//case
			//HWDUBAI_LOGE("DUBAI_TAG_SENSORHUB","name=%s", last_activity_ws->name);
			break;
...
static void iomcu_dubai_log_init(void)
{
	register_mcu_event_notifier(TAG_DUBAI, CMD_DUBAI_SEND_TO_AP, iomcu_dubai_log_process);
	hwlog_info("iomcu_dubai_log_init success\n");
}
#endif
