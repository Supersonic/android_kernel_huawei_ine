...
#define SMART_MP_MARK 10200
static bool   g_Emcom_smartmpOn = false;
static uid_t  g_Emcom_smartmpUid = 0;
#endif
uid_t g_FastSynUid;
...
bool Emcom_Xengine_CheckUidAccount(const struct sk_buff *skb, uint32_t *uid, const struct sock *alternate_sk, int proto)
{
	...
	if (g_Emcom_smartmpOn) {
		smart_mp_sk = skb_to_full_sk(skb);
		if (!smart_mp_sk)
			smart_mp_sk = alternate_sk;
		if (*uid == g_Emcom_smartmpUid) {
			return false;
		} else if (skb->mark == SMART_MP_MARK) {
			*uid = g_Emcom_smartmpUid;
		} else if (smart_mp_sk && sk_fullsock(smart_mp_sk) &&
			   smart_mp_sk->sk_mark == SMART_MP_MARK) {
			*uid = g_Emcom_smartmpUid;
		}
	}
	return true;
...
{
	if (!sk || g_Emcom_smartmpOn == false || proto != IPPROTO_UDP
	    || sk->sk_uid.val != g_Emcom_smartmpUid)
		return true;
	else
		return false;
}
...
void Emcom_Xengine_EvtProc(int32_t event, uint8_t *pdata, uint16_t len)
{
	switch(event)
	{
		...
		case NETLINK_EMCOM_DK_START_SMARTMP:
			...
			if (len >= sizeof(g_Emcom_smartmpUid)) {
				g_Emcom_smartmpOn = true;
				g_Emcom_smartmpUid = *(uid_t *)pdata;
			} else
				EMCOM_LOGD("emcom netlink smartmp len %d err\n", len);
			break;
		case NETLINK_EMCOM_DK_STOP_SMARTMP:
			EMCOM_LOGD("emcom netlink receive smartmp stop\n");
			g_Emcom_smartmpOn = false;
			g_Emcom_smartmpUid = 0;
			break;
#endif
		default:
