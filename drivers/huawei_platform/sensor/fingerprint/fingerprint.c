...
static ssize_t cpufreq_tune_store(struct device* device,
                       struct device_attribute* attribute,
                       const char* buffer, size_t count)
{
    struct fp_data* fingerprint = dev_get_drvdata(device);
    unsigned int time_out = 3000;

    if (NULL == fingerprint)
    {return -EINVAL;}

    unsigned int cpufreq_tune_flag = 0;
    int ret = 0;
    cpufreq_tune_flag = simple_strtoul(buffer, NULL, 10);
    hwlog_err("[%s] cpufreq_tune_flag %d\n", __func__, cpufreq_tune_flag);

    if (cpufreq_tune_flag == 1)
    {
        ret = cpufreq_lowtemp_request(time_out);

        if (ret)
        { hwlog_err("[%s] cpufreq_lowtemp_request error %d\n", __func__, ret); }
...
static DEVICE_ATTR(cpufreq_tune, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, NULL, cpufreq_tune_store);
...
{
    struct fp_data* fingerprint = dev_get_drvdata(device);
    if(NULL == device || NULL == buf || NULL == fingerprint)
    {
        hwlog_err("%s failed,the pointer is null\n", __func__);
        return -EINVAL;
    }
...
{
    struct fp_data* fingerprint = dev_get_drvdata(device);
    if(NULL == device || NULL == buffer || NULL == fingerprint)
    {
        hwlog_err("%s failed,the pointer is null\n", __func__);
        return -EINVAL;
    }
