...
#define DATA_TRANSFER_COFF (8*120)
#define AND_FF (0xFF)
...
uint8_t ROHM_PORT_BH1749_getByte(ROHM_PORT_portHndl * handle, uint8_t reg, uint8_t * data, uint8_t len){
    int ret = ERR_PARA;
...
uint8_t ROHM_PORT_BH1749_setByte(ROHM_PORT_portHndl * handle, uint8_t reg, uint8_t* data, uint8_t len){
	int ret = ERR_PARA;
	if ((handle == NULL) || (data == NULL) ){
		hwlog_err("ROHM_Driver: %s: Pointer is NULL\n", __func__);
		return ERR_PARA;
...
static UINT8 rohm_bh1749_getByte(ROHM_PORT_portHndl * portHndl, uint8_t reg, UINT8 * readData)
{
	UINT8 read_count = 0;
	UINT8 length = 1;
...
static UINT8 rohm_bh1749_setByte(ROHM_PORT_portHndl * portHndl, uint8_t reg, UINT8 data)
{
	UINT8 write_count = 0;
	UINT8 length = 1;
...
static UINT8 rohm_bh1749_getBuf(ROHM_PORT_portHndl * portHndl, uint8_t reg, UINT8 * readData, UINT8 length)
{
	UINT8 read_count = 0;
...
static UINT8 rohm_bh1749_setWord(ROHM_PORT_portHndl * portHndl, uint8_t reg, UINT16 data)
{
	UINT8 write_count = 0;
...
static void rohm_bh1749_resetAllRegisters(rohm_bh1749_deviceCtx_t * ctx, ROHM_PORT_portHndl * portHndl){
	...
	//set 35ms default
	rohm_bh1749_setAtimeInMs(ctx, TIME_35MS);
}
...
static ssize_t osal_als_enable_set(struct colorDriver_chip *chip, uint8_t valueToSet)
{
	...
	if(valueToSet){
		...
		mod_timer(&chip->work_timer, jiffies + msecs_to_jiffies(100));//first enable set the timer as 100ms
		...
		if(chip->calibrationCtx.calSampleCounter >= ROHM_BH1749_CAL_AVERAGE){
			...
			if (result){
				result = (chip->calibrationCtx.calXtarget * (currentGain *
					ROHM_BH1749_FLOAT_TO_FIX / ROHM_BH1749_GAIN_OF_GOLDEN)) / result;
				if(result > ROHM_BH1749_FLOAT_TO_FIX * ROHM_BH1749_CAL_THR ||
					result < ROHM_BH1749_FLOAT_TO_FIX / ROHM_BH1749_CAL_THR ){
					hwlog_err("%s: ratio is out bound[%d, %d]! calXresult[%d] = %d\n" , __func__,
						ROHM_BH1749_FLOAT_TO_FIX*ROHM_BH1749_CAL_THR,
						ROHM_BH1749_FLOAT_TO_FIX/ROHM_BH1749_CAL_THR, chip->calibrationCtx.calState, result);
...
static void  osal_report_als(struct colorDriver_chip *chip)
{
	...
	hwlog_debug(" bh1749 report  data after calibration: x = %d, y = %d, z = %d, ir = %d\n",
		outData.rawX, outData.rawY, outData.rawZ, outData.rawIR);
