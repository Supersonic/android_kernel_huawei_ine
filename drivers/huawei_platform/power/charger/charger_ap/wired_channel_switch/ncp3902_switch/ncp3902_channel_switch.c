...
static int ncp3902_chgsw_set_wired_channel(int flag)
{
	if (use_wireless_switch_cutoff_wired_channel && gpio_chgsw_en) {
		...
		return -ENODEV;
		hwlog_err("%s: ncp3902 channel switch no device!\n", __func__);
	}
	return 0;
}
...
static void ncp3902_chgsw_parse_dts(struct device_node *np)
{
	...
	if (ret) {
		hwlog_err("%s: get use_wireless_switch_cutoff_wired_channel failed\n", __func__);
		use_wireless_switch_cutoff_wired_channel = 0;
	}
	hwlog_info("%s:  use_wireless_switch_cutoff_wired_channel  = %d.\n", __func__, use_wireless_switch_cutoff_wired_channel);
}
...
static int ncp3902_chgsw_probe(struct platform_device *pdev)
{
	...
	if (use_wireless_switch_cutoff_wired_channel) {
		...
		if (ret) {
			hwlog_info("%s ncp3902 channel switch gpio init fail\n", __func__);
			return -1;
		}
		ret = wired_chsw_ops_register(&chsw_ops);
		if (ret) {
			hwlog_err("register ncp3902 switch ops failed!\n");
			gpio_free(gpio_chgsw_en);
			return -1;
		}
		hwlog_info("%s ncp3902 channel switch ops register success\n", __func__);
	}
