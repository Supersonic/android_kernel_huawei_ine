...
#define DIRECT_CHARGE_SC_FUNCTION_IN 	hwlog_info("%s ++\n", __func__);
#define DIRECT_CHARGE_SC_FUNCTION_OUT 	hwlog_info("%s --\n", __func__);
#define DIRECT_CHARGE_SC_FUNCTION_LINE 	hwlog_info("%s [%d] ++\n", __func__, __LINE__);

static struct direct_charge_device *g_di;
static struct loadswitch_ops* g_sc_ops;
static struct batinfo_ops* g_bi_sc_ops;
struct atomic_notifier_head direct_charge_sc_fault_notifier_list;
ATOMIC_NOTIFIER_HEAD(direct_charge_sc_fault_notifier_list);

void direct_charge_sc_get_fault_notifier(struct atomic_notifier_head **notifier)
{
	*notifier = &direct_charge_sc_fault_notifier_list;
}
...
void direct_charge_sc_check(void)
{
	...
	if ((SCP_STAGE_CHARGE_DONE == di->scp_stage) || (CHARGE_DONE == charge_done_status))
	{
		hwlog_err("%s : scp_stage = %d,charge_done_status = %d\n", __func__, di->scp_stage, charge_done_status);
		return;
	}
	if (di->error_cnt >= DC_ERR_CNT_MAX)
	{
		...
		if (FALSE == di->dc_err_report_flag && di->dc_open_retry_cnt <= DC_OPEN_RETRY_CNT_MAX) {
			...
			dsm_report(DSM_DIRECT_CHARGE_ERR, di->dc_err_dsm_buff);
...
static void direct_charge_fault_work(struct work_struct *work)
{
	...
	case DIRECT_CHARGE_FAULT_VBUS_OVP:
		...
		break;
	case DIRECT_CHARGE_FAULT_TSBAT_OTP:
		hwlog_err("tsbat otp happened!\n");
		snprintf(buf, sizeof(buf), "tsbat otp happened\n");
		strncat(buf, reg_info, strlen(reg_info));
		//dsm_report(DSM_DIRECT_CHARGE_TSBAT_OTP, buf);
		break;
	case DIRECT_CHARGE_FAULT_TSBUS_OTP:
		hwlog_err("tsbus otp happened!\n");
		snprintf(buf, sizeof(buf), "tsbus otp happened\n");
		strncat(buf, reg_info, strlen(reg_info));
		//dsm_report(DSM_DIRECT_CHARGE_TSBUS_OTP, buf);
		break;
	case DIRECT_CHARGE_FAULT_TDIE_OTP:
		hwlog_err("tdie otp happened!\n");
		snprintf(buf, sizeof(buf), "tdie otp happened\n");
		strncat(buf, reg_info, strlen(reg_info));
		//dsm_report(DSM_DIRECT_CHARGE_TDIE_OTP, buf);
		break;
...
static struct direct_charge_sysfs_field_info direct_charge_sysfs_field_tbl[] = {
	...
	DIRECT_CHARGE_SYSFS_FIELD_RW(set_chargetype_priority, SET_CHARGETYPE_PRIORITY),
};
				  ...
				  const char *buf, size_t count)
{
	...
	case DIRECT_CHARGE_SYSFS_SET_CHARGETYPE_PRIORITY:
		if ((strict_strtol(buf, 10, &val) < 0) || (val < 0) || (val > MAX_RESISTANCE))
			return -EINVAL;
		hwlog_info("set chargertype priority = %ld\n", val);

		if (SCP_CHARGE_TYPE_SC == val)
			direct_charge_or_set_local_mode(SC_MODE);//sc
		else if (SCP_CHARGE_TYPE_LVC == val)
			direct_charge_and_set_local_mode(LVC_MODE);//lvc
		else 
			hwlog_info("unsupport chargertype priority \n");
		break;
	default:
		hwlog_err("(%s)NODE ERR!!HAVE NO THIS NODE:(%d)\n", __func__, info->name);
		break;
...
int batinfo_sc_ops_register(struct batinfo_ops* ops)
{
	...
	if (ops != NULL)
	{
		...
		hwlog_err("batinfo sc ops register fail!\n");
		ret = -EPERM;
	}
	return ret;
}
...
static int direct_charge_sc_probe(struct platform_device	*pdev)
{
	int ret = 0;
	...
	battery_aging_safe_policy(di);//need move
	...
	direct_charge_or_set_local_mode(SC_MODE);

	platform_set_drvdata(pdev, di);

	di->direct_charge_wq = create_singlethread_workqueue("direct_charge_sc_wq");
	di->direct_charge_watchdog_wq = create_singlethread_workqueue("direct_charge_sc_watchdog_wq");
	...
	if (ret)
		hwlog_err("can't create	sc charge sysfs entries\n");
	power_class = hw_power_get_class();
	if (power_class)
	{
		if (charge_dev == NULL)
			charge_dev = device_create(power_class,	NULL, 0, NULL, "charger");
		ret = sysfs_create_link(&charge_dev->kobj, &di->dev->kobj, "direct_charger_sc");
		if (ret)
		{
	...
	ret = atomic_notifier_chain_register(&direct_charge_sc_fault_notifier_list, &di->fault_nb);
	if (ret < 0)
	{
	...
	if (di->ls_id < 0 || di->ls_id >= SWITCHCAP_TOTAL)
	{
		hwlog_err("error switchcap id info\n");
		di->ls_id = SWITCHCAP_TOTAL;
	}
	di->ls_name = switchcap_name[di->ls_id];
	hwlog_info("switchcap id = %d(%s)\n", di->ls_id, di->ls_name);
	ret |= di->ls_ops->ls_exit();
	if (ret)
	{
		hwlog_err("dev_check for switchcap fail.\n");
	}
	else
	{
		set_hw_dev_flag(DEV_I2C_SWITCHCAP);
		hwlog_info("dev_check for switchcap succ.\n");
	}
#endif
	hwlog_info("direct charger sc probe ok!\n");
	DIRECT_CHARGE_SC_FUNCTION_OUT
	return	0;
...
*	return value: NULL
**********************************************************/
static int direct_charge_sc_remove(struct platform_device *pdev)
{
	DIRECT_CHARGE_SC_FUNCTION_IN
...
*	return value: NULL
**********************************************************/
static void direct_charge_sc_shutdown(struct platform_device *pdev)
{
	DIRECT_CHARGE_SC_FUNCTION_IN
	struct direct_charge_device *di = platform_get_drvdata(pdev);
...
*	return	value: 0-sucess	or others-fail
**********************************************************/
static int direct_charge_sc_suspend(struct	platform_device	*pdev, pm_message_t state)
{
	return	0;
}
...
*	return	value: 0-sucess	or others-fail
**********************************************************/
static int direct_charge_sc_resume(struct	platform_device	*pdev)
{
	return	0;
}
#endif	/* CONFIG_PM */

static struct of_device_id direct_charge_sc_match_table[] = {
	{
...
static struct platform_driver direct_charge_sc_driver = {
	.probe = direct_charge_sc_probe,
	.remove	= direct_charge_sc_remove,
#ifdef	CONFIG_PM
	.suspend = direct_charge_sc_suspend,
	.resume	= direct_charge_sc_resume,
#endif
	.shutdown = direct_charge_sc_shutdown,
	.driver	= {
		.name = "direct_charger_sc",
		.owner = THIS_MODULE,
		.of_match_table	= of_match_ptr(direct_charge_sc_match_table),
	},
};
...
*	return value: 0-sucess or others-fail
**********************************************************/
static int __init direct_charge_sc_init(void)
{
	DIRECT_CHARGE_SC_FUNCTION_LINE
	return platform_driver_register(&direct_charge_sc_driver);
}
...
*	return	value:	NULL
**********************************************************/
static void __exit direct_charge_sc_exit(void)
{
	DIRECT_CHARGE_SC_FUNCTION_LINE
	platform_driver_unregister(&direct_charge_sc_driver);
}
/*lint -restore*/

late_initcall(direct_charge_sc_init);
module_exit(direct_charge_sc_exit);
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("direct charger sc module driver");
