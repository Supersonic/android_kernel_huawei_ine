...
#define DIRECT_CHARGE_LVC_FUNCTION_IN 	hwlog_info("%s ++\n", __func__);
#define DIRECT_CHARGE_LVC_FUNCTION_OUT 	hwlog_info("%s --\n", __func__);
#define DIRECT_CHARGE_LVC_FUNCTION_LINE 	hwlog_info("%s [%d] ++\n", __func__, __LINE__);
...
static int direct_charge_lvc_probe(struct platform_device	*pdev)
{
	...
	DIRECT_CHARGE_LVC_FUNCTION_OUT
	return	0;
...
{
	DIRECT_CHARGE_LVC_FUNCTION_LINE
