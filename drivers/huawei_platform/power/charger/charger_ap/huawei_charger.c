...
#ifdef CONFIG_DIRECT_CHARGER
	if (is_direct_charge_failed() && (FCP_STAGE_SUCESS != fcp_get_stage_status()) && (FCP_STAGE_RESET_ADAPTOR != fcp_get_stage_status())) {
		fcp_set_stage_status(FCP_STAGE_DEFAUTL);
	}
#endif
...
static void charge_parse_dts(struct charge_device_info *di)
{
	...
	ret = of_property_read_u32(of_find_compatible_node(NULL, NULL, "huawei,charger"),
			"disable_monitor_term", &di->dis_monitor_term);
	if (ret){
		di->dis_monitor_term = 0;
		hwlog_err("get disable_monitor_term fail.\n");
	}
	hwlog_info("dis_monitor_term = %d\n",di->dis_monitor_term);
...
int charge_otg_mode_enable(int enable)
{
	if (g_di)
	{
		if (enable)
