...
int huawei_get_charge_state(void)
{
	...
	if (di == NULL || di->ops == NULL ||
	    ...
	    di->ops->check_input_vdpm_state == NULL ||
	    di->ops->check_input_idpm_state == NULL) {
		hwlog_err("coul_get_charge_state di is NULL\n", __func__);
		return FALSE;
	}

	ret = di->ops->get_charge_state(&state);
	if (ret < 0) {
		hwlog_err("coul_get_charge_state fail, ret = 0x%x\n", ret);
		return FALSE;
	}

	if ((state != CHAGRE_STATE_NORMAL) ||
	    di->ops->check_input_vdpm_state() ||
...
enum huawei_usb_charger_type huawei_get_charger_type(void)
{
	struct charge_device_info *di = g_di;
...
/**********************************************************
*  Function:       power_send_event_to_user
*  Description:    send uevent to userspace
*  Parameters:     event_tag: identify of the event
*                  event_info: content of the event
*  return value:   void
**********************************************************/
void power_send_event_to_user(char *event_tag, char *event_info)
{
	char event_buf[POWER_UEVENT_SIZE] = {0};
	char *envp[] = { event_buf, NULL };
	int event_len;

	if (event_tag == NULL || event_info == NULL || g_di == NULL) {
		hwlog_err("%s input invalid\n", __func__);
		return;
	}

	event_len = strlen(event_tag) + strlen(event_info);
	/* reserve 3 bytes for ": " and '\0' */
	if (event_len + 3 >  POWER_UEVENT_SIZE) {
		hwlog_err("%s: event is too long to send\n", __func__);
		return;
	}
	snprintf(event_buf, sizeof(event_buf), "%s: %s", event_tag, event_info);
	hwlog_info("%s: %s \n", event_tag, event_info);
	kobject_uevent_env(&(g_di->dev->kobj), KOBJ_CHANGE, envp);
}
/*lint -save -e* */
#ifdef CONFIG_SYSFS
