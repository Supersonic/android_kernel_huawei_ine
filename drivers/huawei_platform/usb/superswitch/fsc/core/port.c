...
#if defined(CONFIG_HUAWEI_DSM)
#define DSM_MAX_SIZE 1024
static struct dsm_dev dsm_superswitch = {
	.name = "superswitch",
	.fops = NULL,
	.buff_size = DSM_MAX_SIZE,
};

static struct dsm_client *superswitch_dclient = NULL;

struct dsm_client *get_superswitch_dclient(void)
{
	return superswitch_dclient;
}
void superswitch_dsm_report(int num)
{
	...
	if (NULL == superswitch_dclient) {
		pr_err("FUSB  %s - Error: superswitch_dclient is NULL!\n", __func__);
		return;
	}
	snprintf(dsm_buff, sizeof(dsm_buff), "tc_state= 0x%x\n", port->tc_state_);
	if (!dsm_client_ocuppy(superswitch_dclient)) {
		...
		dsm_client_record(superswitch_dclient, "%s", dsm_buff);
		dsm_client_notify(superswitch_dclient, num);
	}
}
#endif
...
#if defined(CONFIG_HUAWEI_DSM)
	if (!superswitch_dclient)
		superswitch_dclient = dsm_register_client(&dsm_superswitch);
