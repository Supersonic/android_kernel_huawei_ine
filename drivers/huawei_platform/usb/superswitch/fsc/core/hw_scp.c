...
static int FUSB3601_scp_init(struct scp_init_data * sid)
{
	...
	if (get_dpd_enable()) {
		...
		if (IWATT_ADAPTER == ret || WELTREND_ADAPTER == ret || ID0X32_ADAPTER == ret) {
			DmRd_enable(DMRD_ENABLE, DmRd_direct_charge);
			hwlog_info("%s :  config rd on Dm for IWATT\n ",__func__);
		}else if (RICHTEK_ADAPTER == ret) {
			hwlog_info("%s :  disable rd on Dm for RICHTEK\n ",__func__);
			DmRd_enable(DMRD_DISABLE, DmRd_redobc12);
			DmRd_enable(DMRD_DISABLE, DmRd_hardreset);
		}
#endif
...
static int FUSB3601_scp_exit(struct direct_charge_device* di)
{
	...
	if (get_dpd_enable()) {
		...
		DmRd_enable(DMRD_DISABLE, DmRd_direct_charge);
#endif
