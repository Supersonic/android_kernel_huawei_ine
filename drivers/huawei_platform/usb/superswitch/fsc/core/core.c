...
#ifdef CONFIG_USB_ANALOG_HS_INTERFACE
static unsigned int DmRd_status = 0;
static DEFINE_MUTEX(DmRd_op_mutex);
#endif
...
#ifdef CONFIG_USB_ANALOG_HS_INTERFACE
static int DmRd_set_enable(DmRd_type type)
{
	if (DmRd_status == 0)
		usb_analog_hs_plug_in_out_handle(DIRECT_CHARGE_IN);
	DmRd_status = DmRd_status | (1 << type);

	hwlog_info("%s: DmRd enable(%d) success!\n", __func__, type);
	return 0;
}
static int DmRd_set_disable(DmRd_type type)
{
	if (DmRd_status != 0) {
		DmRd_status = DmRd_status & (~(1 << type));
		if (DmRd_status == 0) {
	...
	hwlog_info("%s: DmRd disable(%d) success!\n", __func__, type);
	return 0;

}

int DmRd_enable(bool enable, DmRd_type type)
{
	hwlog_info("%s: count(%d), type(%d), value(%d)\n", __func__, DmRd_status, type, enable);

	if (type >= DmRd_CTRL_MAX) {
		hwlog_err("%s: type(%d) is invalid!\n", __func__, type);
		return -EINVAL;
	}

	mutex_lock(&DmRd_op_mutex);
	if (enable) {
		DmRd_set_enable(type);
	} else {
		DmRd_set_disable(type);
	}
	mutex_unlock(&DmRd_op_mutex);
...
#ifdef CONFIG_USB_ANALOG_HS_INTERFACE
	DmRd_enable(DMRD_ENABLE, DmRd_redobc12);
#endif
	if((port->tc_state_ == AttachedSink) || (port->tc_state_ == DebugAccessorySink)) {
...
#ifdef CONFIG_USB_ANALOG_HS_INTERFACE
	DmRd_enable(DMRD_DISABLE, DmRd_redobc12);
#endif
