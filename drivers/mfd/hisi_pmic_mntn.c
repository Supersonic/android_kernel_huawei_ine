...
static int hisi_pmic_ocp_special_handler(char *power_name, PMIC_MNTN_DESC *pmic_mntn)
{
	...
	do {
		if (!strcmp(cur->power_name, power_name)) {/*lint !e421*/
...
static void get_pmu_key_register_info(PMIC_MNTN_DESC *pmic_mntn)
{
	...
	for (i = 0; i < pmic_mntn->pmu_record_reg_n; i++) {/*lint !e574*/
...
static int hisi_pmic_get_batt_info(int *vbatt, char **batt_brand, int *batt_temp, int *capacity)
{
...
static void hisi_pmic_ocp_scan(PMIC_MNTN_DESC *pmic_mntn)
{
	...
	for (index = 0; index < reg_num; index++) {
		...
		if (reg_buff[index]) {
			...
			for (bit = 0; bit < pmic_mntn->data_width; bit++) {/*lint !e574*/
...
static void hisi_pmic_record_events(PMIC_MNTN_DESC *pmic_mntn)
{
	...
	for (index = 0; index < pmic_mntn->record_reg_n; index++) {/*lint !e574*/
		...
		if (pmic_mntn->record_event_buff[index] & HISI_PMIC_DSM_MASK_STATE) {
			for (i = 0; i < pmic_mntn->data_width; i++) {/*lint !e574*/
		...
		snprintf(pmic_mntn->init_log_show + strlen(pmic_mntn->init_log_show), PMIC_PRINT_BUF_SIZE,
				"  0x%0x    0x%0x \n", pmic_mntn->record_regs[index], pmic_mntn->record_event_buff[index]);/*lint !e567*/
	}
...
static void hisi_pmic_clear_ocp(PMIC_MNTN_DESC *pmic_mntn)
{
	...
	for (index = 0; index < pmic_mntn->ocp_reg_n; index++) {/*lint !e574*/
...
static int hisi_ocp_mold_switch_initial(struct platform_device *pdev, PMIC_MNTN_DESC *pmic_mntn)
{
	...
	ret = of_property_read_u32(np, "hisilicon,ocp-mold-set-switch", (u32 *)&pmic_mntn->ocp_mold_switch);
	if (ret) {
	...
	for (i = 0; i < pmic_mntn->ocp_ctrl_n; i++) {/*lint !e574*/
...
static void uv_mntn_work(struct work_struct *work)
{
	...
	if (kstrtol(battery_capacity_info, 0, &battery_capacity) < 0)
		pr_err("[%s] kstrtol fail \n", __func__);
...
#if (LINUX_VERSION_CODE > KERNEL_VERSION(3, 13, 0))
	ret = devm_request_irq(dev, pmic_mntn->uv_irq, hisi_pmic_uv_irq_handler, IRQF_NO_SUSPEND,
