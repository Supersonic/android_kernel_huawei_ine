					...
					int range_size, int row_size)
{
	...
	if(rx_num <= 0 || rx_num > RAWDATA_NUM_OF_TRX_MAX || tx_num <= 0 || tx_num > RAWDATA_NUM_OF_TRX_MAX){
	...
	if((index1 <= TS_RAWDATA_RESULT_MAX/2)
		...
		if(g_ts_kit_platform_data.chip_data->support_forcekey_cap_value_test){
			for (index = CAP_TEST_FORCEKEY_VALUE_NUM;  index < rx_num + CAP_TEST_FORCEKEY_VALUE_NUM; index++) {
		...
		seq_printf(m, "\ntx:\n");
		if(g_ts_kit_platform_data.chip_data->support_forcekey_cap_value_test){
			for (index = CAP_TEST_FORCEKEY_VALUE_NUM; index < tx_num + CAP_TEST_FORCEKEY_VALUE_NUM; index++) {
...
static int fb_notifier_callback(struct notifier_block* self, unsigned long event, void* data)
{
    ...
    TS_LOG_INFO("tpkit fb_callback called,event is %d\n",event);
