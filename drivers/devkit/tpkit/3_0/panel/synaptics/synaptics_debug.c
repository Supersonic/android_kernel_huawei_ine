...
static void check_forcekey_cap_value(void)
{
	...
	if(0 != result){
	     TS_LOG_ERR("%s(%d), ts_kit_parse_csvfile  parse tx fail result=%dis 10F\n", __func__, __LINE__, result);
	     goto test_fail;
	}
	...
	for(i = 0; i < CAP_TEST_FORCEKEY_VALUE_NUM; i++){
		if(false == is_in_range(f54->forcekey_value[i],
		    ...
		    TS_LOG_ERR("%s(%d), hybrid_abs_delta test is out of range, test result is 10F\n", __func__, __LINE__);
		    strncpy(tp_test_failed_reason, "-panel_reason",
			TP_TEST_FAILED_REASON_LEN);
		    goto test_fail;
		}
	...
	TS_LOG_INFO("%s(%d), forcekey   test result is 10P \n",__func__, __LINE__);
...
static int mmi_hybrid_raw_cap_test(void)
{
	...
	if (result < 0) {
		if(f54->rmi4_data->synaptics_chip_data->support_forcekey_cap_value_test){
			TS_LOG_ERR("%s(%d), Failed to get data\n", __func__, __LINE__);
			strncat(buf_f54test_result, "-6F-10F", MAX_STR_LEN);
		}
		else{
			TS_LOG_ERR("%s(%d), Failed to get data\n", __func__, __LINE__);
			strncat(buf_f54test_result, "-6F", MAX_STR_LEN);
...
static int mmi_ext_trx_short_test(void)
{
	...
	for (i = 0; i< 4; i++) {
		...
		if(f54->rmi4_data->synaptics_chip_data->ext_trex_short_wait_report_delay_flag)
			msleep(200);
...
int synap_get_cap_data(struct ts_rawdata_info *info)
{
	...
	if (rc < 0) {		/*i2c communication failed, mmi result is all failed */
		...
		if(f54->rmi4_data->synaptics_chip_data->self_cap_test)
		{
			...
			if(f54->rmi4_data->synaptics_chip_data->support_forcekey_cap_value_test && rc >= 0)
			{
