...
static void synap_parse_chip_oem_specific_dts(struct ts_kit_device_data *chip_data, struct device_node *device)
{
	 int retval = NO_ERR;
	 int value = 0;
	 retval = of_property_read_u32(device, SUPPORT_2DBARCODE_INFO, &chip_data->support_2dbarcode_info);
	 if (retval) {
		 TS_LOG_ERR("%s: support_2dbarcode_info has NOT been set\n", __func__);
		 chip_data->support_2dbarcode_info = 0;
	 }
	 TS_LOG_INFO("%s: support_2dbarcode_info = %d\n", __func__, chip_data->support_2dbarcode_info);
	 return;
}
...
void  synap_parse_chip_specific_dts(struct ts_kit_device_data *chip_data)
{
	...
	retval = of_property_read_u32(device, EXT_TREX_SHORT_TEST_WAIT_REPORT_DELAY_FLAG, \
							&chip_data->ext_trex_short_wait_report_delay_flag);
	if (retval) {
		TS_LOG_ERR("%s: ext_trex_short_wait_report_delay_flag has NOT been set\n", __func__);
		chip_data->ext_trex_short_wait_report_delay_flag = 0;
	}
	TS_LOG_INFO("%s: ext_trex_short_wait_report_delay_flag = %d\n", __func__, \
				chip_data->ext_trex_short_wait_report_delay_flag);

	retval = of_property_read_u32(device, SUPPORT_FORCEKEY_CAP_VALUE_TEST, &chip_data->support_forcekey_cap_value_test);
	if (retval) {
		TS_LOG_ERR("%s: support_forcekey_cap_value_test has NOT been set\n", __func__);
		chip_data->support_forcekey_cap_value_test = 0;
	}
	TS_LOG_INFO("%s: support_forcekey_cap_value_test = %d\n", __func__, chip_data->support_forcekey_cap_value_test);
	...
	synap_parse_chip_oem_specific_dts(chip_data, device);
...
static int synaptics_chip_detect(struct ts_kit_platform_data *data)
{
	...
	if(true == rmi4_data->synaptics_chip_data->check_bulcked){
		if((tp_ops)&&(tp_ops->get_status_by_type)) {
			retval = tp_ops->get_status_by_type(PT_STATION_TYPE, &isbulcked);
			return retval;
		}
		if(false == isbulcked){
	...
	if(rmi4_data->synaptics_chip_data->LDO29_980_default_on_flag){
		synatpics_vddio_off();
		mdelay(6);
	}
