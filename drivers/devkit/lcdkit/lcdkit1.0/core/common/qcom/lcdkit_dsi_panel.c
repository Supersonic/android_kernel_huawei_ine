...
void mdss_dsi_panel_bklt_dcs(void *pdata, int bl_level)
{
	...
	/* closed dimming and backlight for closed backlight slow issue*/
	if((0 == bl_level)
		&& (NULL !=lcdkit_info.panel_infos.dis_pandimming_bf_sleepin_flag)
		&& (NULL != &lcdkit_info.panel_infos.disable_pan_dimming_cmds.cmds)
	...
	if ((0 != bl_level)
		&& (DIMMING_DISABLE == lcdkit_pan_dimming_state)
		&& (NULL !=lcdkit_info.panel_infos.dis_pandimming_bf_sleepin_flag)
		&& (NULL != &lcdkit_info.panel_infos.enable_pan_dimming_cmds.cmds))
	{
