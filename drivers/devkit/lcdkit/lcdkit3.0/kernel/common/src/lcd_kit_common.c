...
static char* cmd_array[SENCE_ARRAY_SIZE] = {
	...
	"/sys/class/graphics/fb0/lcd_checksum",
	"/sys/class/graphics/fb0/lcd_checksum",
	"/sys/class/lcd_backlight/lcd_backlight/self_test",
...
{
	if (enable) {
		lcd_kit_ts_resume(sync);
	} else {
		lcd_kit_ts_early_suspend(sync);
	}
}
...
{
	if (enable) {
		lcd_kit_ts_after_resume(sync);
	} else {
		lcd_kit_ts_suspend(sync);
	}
}
...
static int lcd_kit_event_should_send(uint32_t event)
{
	int ret = 0;
	...
	switch (event) {
		...
		case EVENT_RESET:
		case EVENT_VDD:
		return (lcd_kit_get_pt_mode()||lcd_kit_gesture_mode());
		default:
		ret = 0;
		break;
	}
...
int lcd_kit_event_handler(void* hld, uint32_t event, uint32_t data, uint32_t delay)
{
	...
	if (lcd_kit_event_should_send(event)) {
		return ret;
	}
	switch (event) {
		case EVENT_VCI:
...
static int lcd_kit_hbm_set_handle(void* hld, uint32_t level)
{
	int ret = LCD_KIT_OK;
	struct lcd_kit_adapt_ops* adapt_ops = NULL;
	...
	if (common_info->hbm.support) {
		if(level != 0) {
			if (adapt_ops->mipi_tx) {
				ret = adapt_ops->mipi_tx(hld, &common_info->hbm.prepare_cmds);
			}
			...
			if (adapt_ops->mipi_tx) {
				ret = adapt_ops->mipi_tx(hld, &common_info->hbm.post_cmds);
			}
		} else {
			if (adapt_ops->mipi_tx) {
...
static int lcd_kit_get_panel_info(char* buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "blmax:%u,blmin:%u,blmax_nit_actual:%d,blmax_nit_standard:%d,lcdtype:%s,\n",
				   ...
				   bl_max_nit, common_info->bl_max_nit, panel_type);
	return ret;
}
...
static int lcd_kit_check_reg(void* hld, char* buf)
{
	...
	if (common_info->check_reg.support) {
		expect_ptr = common_info->check_reg.value.buf;
...
static int lcd_kit_set_ce_mode(void* hld, int mode)
{
	int ret = LCD_KIT_OK;
	struct lcd_kit_adapt_ops* adapt_ops = NULL;
...
{
	if (!vpos || !vneg) {
		LCD_KIT_ERR("vpos is %s, vneg is %s\n", vpos, vneg);
		return LCD_KIT_FAIL;
	}
