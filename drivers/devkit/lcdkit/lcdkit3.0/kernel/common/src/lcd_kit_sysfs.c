...
static ssize_t lcd_kit_amoled_pcd_errflag_check(struct device* dev,
		struct device_attribute* attr, char* buf)
{
	ssize_t ret = LCD_KIT_OK;
	struct lcd_kit_sysfs_ops* sysfs_ops = NULL;
...
static DEVICE_ATTR(amoled_pcd_errflag_check, 0644, lcd_kit_amoled_pcd_errflag_check, NULL);
...
int lcd_kit_create_sysfs(struct kobject* obj)
{
	...
	for (i = 0; i < sizeof(lcd_kit_conf)/sizeof(lcd_kit_conf[0]); i++) {
		if (i >= (LCD_KIT_SYSFS_MAX - 1)) {
			LCD_KIT_ERR("dev attr number exceed sysfs max\n");
			return LCD_KIT_FAIL;
		}
		if (lcd_kit_check_support(i)) {
