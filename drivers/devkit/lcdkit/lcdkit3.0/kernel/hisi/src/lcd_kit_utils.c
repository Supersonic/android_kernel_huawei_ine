...
int lcd_kit_read_project_id(void)
{
	...
	if (disp_info->project_id.support) {
		ret = lcd_kit_dsi_cmds_rx(hisifd, (uint32_t*)disp_info->project_id.id, &disp_info->project_id.cmds);
		if (ret) {
...
int lcd_kit_rgbw_set_mode(struct hisi_fb_data_type* hisifd, int mode)
{
	...
	switch (hisifd->de_info.ddic_rgbw_mode) {
		case RGBW_SET1_MODE:
...
int lcd_kit_rgbw_set_backlight(struct hisi_fb_data_type* hisifd, int bl_level)
{
	...
	disp_info->rgbw.backlight_cmds.cmds[0].payload[1] = bl_level & 0xff;
...
int lcd_kit_rgbw_set_handle(struct hisi_fb_data_type* hisifd)
{
	...
	/*set mode*/
	if (old_rgbw_mode != hisifd->de_info.ddic_rgbw_mode) {
		ret = lcd_kit_rgbw_set_mode(hisifd, hisifd->de_info.ddic_rgbw_mode);
		if (ret) {
			HISI_FB_ERR("set mode fail\n");
			return LCD_KIT_FAIL;
		}
		old_rgbw_mode = hisifd->de_info.ddic_rgbw_mode;
	}
	/*set backlight*/
	if (old_rgbw_backlight != hisifd->de_info.ddic_rgbw_backlight) {
		ret = lcd_kit_rgbw_set_backlight(hisifd, hisifd->de_info.ddic_rgbw_backlight);
		if (ret) {
			HISI_FB_ERR("set backlight fail\n");
			return LCD_KIT_FAIL;
		}
		old_rgbw_backlight = hisifd->de_info.ddic_rgbw_backlight;
	}
	return ret;
}
...
int lcd_kit_updt_fps(struct platform_device* pdev)
{
	...
	switch (pinfo->fps_scence) {
		case LCD_FPS_SCENCE_FUNC_DEFAULT_DISABLE:
			ret = lcd_kit_dsi_cmds_tx(hisifd, &disp_info->fps.fps_to_60_cmds);
			pinfo->fps_updt_support = 0;
			pinfo->fps_updt_panel_only = 0;
			break;
		case LCD_FPS_SCENCE_FUNC_DEFAULT_ENABLE:
			ret = lcd_kit_dsi_cmds_tx(hisifd, &disp_info->fps.dfr_enable_cmds);
			break;
		case LCD_FPS_SCENCE_FORCE_30FPS:
			ret = lcd_kit_dsi_cmds_tx(hisifd, &disp_info->fps.fps_to_30_cmds);
			break;
		default:
			break;
	}
...
void lcd_kit_get_calicolor_data(struct lcd_kit_oem_info* oem_info)
{
	void* vir_addr = 0;
	if (oem_info == NULL) {
		LCD_KIT_ERR("oem_info is NULL!\n");
		return;
	}
	...
	memcpy((void*)oem_info, (void*)vir_addr, sizeof(struct lcd_kit_oem_info));
	iounmap(vir_addr);
...
void lcd_kit_pinfo_init(struct device_node* np, struct hisi_panel_info* pinfo)
{
	...
	OF_PROPERTY_READ_U8_RETURN(np, "lcd-kit,prefix-sharp-one-d-support", &pinfo->prefix_sharpness1D_support);
	OF_PROPERTY_READ_U8_RETURN(np, "lcd-kit,prefix-sharp-two-d-support", &pinfo->prefix_sharpness2D_support);
...
void lcd_kit_parse_effect(struct device_node* np)
{
	...
	/*color coordinate*/
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,col-coordinate-support", &disp_info->oeminfo.col_coordinate.support, 0);
	if (disp_info->oeminfo.col_coordinate.support) {
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,col-coordinate-cmds", "lcd-kit,col-coordinate-cmds-state",
							   &disp_info->oeminfo.col_coordinate.cmds);
	}
	/*oem information*/
...
int lcd_kit_checksum_check(struct hisi_fb_data_type* hisifd)
{
	...
	int i = 0;
	uint32_t read_value[LCD_KIT_CHECKSUM_SIZE] = {0};
...
int lcd_kit_read_gamma(struct hisi_fb_data_type* hisifd, uint8_t *read_value)
{
	...
	lcd_kit_dsi_cmds_rx(hisifd, (uint32_t*)read_value, &disp_info->gamma_cal.cmds);
	return ret;
}
...
int lcd_kit_utils_init(struct device_node* np, struct hisi_panel_info* pinfo)
{
	...
	if (disp_info->oeminfo.support) {
		//lcd_kit_get_calicolor_data(&disp_info->col_oeminfo.oem_info);
	}
	return LCD_KIT_OK;
}
