...
static int smartpakit_do_write_regs(smartpakit_i2c_priv_t *i2c_priv, unsigned int num, smartpakit_param_node_t *regs)
{
	...
	for (i = 0; i < (int)num; i++) {
		...
		} else if (regs[i].node_type == SMARTPAKIT_PARAM_NODE_TYPE_AW_CORRECT) {
			ret = aw_smartpa_set_correction(cfg->regmap, regs[i].index,
				regs[i].mask, regs[i].value);
