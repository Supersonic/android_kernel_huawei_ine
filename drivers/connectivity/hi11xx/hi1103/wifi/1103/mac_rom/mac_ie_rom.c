...
oal_uint32  mac_proc_ht_opern_ie(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_payload, mac_user_stru *pst_mac_user)
{
    ...
    if (puc_payload[1] < 6)
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{mac_proc_ht_opern_ie::invalid ht opern ie len[%d].}", puc_payload[1]);
        return ul_change;
    }
    ...
    **************************************************************************/
        ...
        (uc_sta_chan_width_old           != st_ht_hdl.bit_sta_chan_width))
    {
         ...
         OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "mac_proc_ht_opern_ie:usr_bw is updated second_chan_offset from [%d] to [%d], chan_with from [%d] to [%d]",
...
oal_uint32  mac_ie_proc_vht_opern_ie(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_payload, mac_user_stru *pst_mac_user)
{
    ...
    {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{mac_ie_proc_vht_opern_ie::invalid channel width[%d], use 20M chn width.}", st_vht_hdl.en_channel_width);
    ...
    mac_user_set_vht_hdl(pst_mac_user, &st_vht_hdl);

    if ((en_channel_width_old != st_vht_hdl.en_channel_width) ||
        (uc_channel_center_freq_seg0_old != st_vht_hdl.uc_channel_center_freq_seg0))
    {
        uc_ret =  MAC_VHT_CHANGE;
        OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "mac_ie_proc_vht_opern_ie:usr_bw is updated chanl_with from [%d] to [%d], chanl_center_freq_seg0 from [%d] to [%d]",
