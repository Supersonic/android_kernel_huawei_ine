                ...
                oal_uint16                      us_type)
{
    ...
    if (OAL_PTR_NULL == pst_hmac_ap)
    {
        OAM_ERROR_LOG1(pst_mac_ap->uc_vap_id, OAM_SF_AUTH,
            "{hmac_mgmt_encap_asoc_rsp_ap::pst_hmac_vap[%d] is NULL!}", pst_mac_ap->uc_vap_id);
    ...
    if (OAL_TRUE == pst_mac_user->st_vht_hdl.en_vht_capable)
    {
        ...
        mac_set_opmode_notify_ie((oal_void *)pst_mac_ap, puc_asoc_rsp, &uc_ie_len);
        puc_asoc_rsp += uc_ie_len;
#endif
    ...
    mac_add_app_ie((oal_void *)pst_mac_ap, puc_asoc_rsp, &us_app_ie_len, OAL_APP_VENDOR_IE);
    puc_asoc_rsp += us_app_ie_len;
#endif
    ...
    mac_set_rrm_enabled_cap_field((oal_void *)pst_mac_ap, puc_asoc_rsp, &uc_ie_len);
    puc_asoc_rsp += uc_ie_len;
#endif
    /* multi-sta特性下新增4地址ie */
    ...
    OAM_WARNING_LOG0(0, OAM_SF_ASSOC, "{hmac_mgmt_encap_asoc_rsp_ap::encap assoc rsp!}\r\n");
#endif
    ...
    *puc_auth_resend = OAL_FALSE;
    ...
    if (OAL_SUCC == mac_chip_find_user_by_macaddr(pst_hmac_vap->st_vap_base_info.uc_chip_id, puc_mac_addr, &us_user_idx))
    {
        ...
        if (OAL_PTR_NULL != pst_user)
        {
            ...
            {
                /* 抛事件上报内核，已经删除某个STA */
                hmac_mgmt_send_disassoc_frame(&(pst_hmac_other_vap->st_vap_base_info), puc_mac_addr, MAC_DISAS_LV_SS, OAL_FALSE);
...
oal_uint16  hmac_encap_auth_rsp(mac_vap_stru *pst_mac_vap, oal_netbuf_stru *pst_auth_rsp, oal_netbuf_stru *pst_auth_req, oal_uint8 *puc_chtxt)
{
    ...
    OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_CONN, "{hmac_encap_auth_rsp:: AUTH_RSP tx : user mac:%02X:XX:XX:%02X:%02X:%02X}",
...
#ifdef _PRE_DEBUG_MODE
    OAM_WARNING_LOG0(0, OAM_SF_AUTH, "{hmac_encap_auth_rsp::encap auth!}\r\n");
#endif
