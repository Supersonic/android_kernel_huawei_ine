...
oal_uint32  hmac_cfg80211_start_scan_sta(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    pst_app_ie->ul_ie_len = pst_cfg80211_scan_param_pst->pst_mac_cfg80211_scan_param->ul_ie_len;
    oal_memcopy(pst_app_ie->auc_ie, pst_cfg80211_scan_param_pst->pst_mac_cfg80211_scan_param->puc_ie, pst_app_ie->ul_ie_len);
...
#endif
    /* 统一在外层释放层alloc的内存 避免下面fsm失败时候外层重复释放内存 */
...
oal_void  hmac_handle_join_rsp_sta(hmac_vap_stru *pst_hmac_vap, oal_uint8 *puc_msg)
{
    ...
    if (HMAC_MGMT_SUCCESS == pst_join_rsp->en_result_code)
    {
        ...
        hmac_chr_set_connect_code(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAL_SUCC);
#endif
    }
    else
    {
...
#ifdef _PRE_WLAN_1103_CHR
        hmac_chr_set_connect_code(pst_hmac_vap->st_vap_base_info.uc_vap_id, MAC_JOIN_RSP_TIMEOUT);
#endif
...
#ifdef _PRE_WLAN_1103_CHR
    hmac_chr_set_connect_code(pst_hmac_vap->st_vap_base_info.uc_vap_id, pst_auth_rsp->us_status_code);
#endif
...
oal_void  hmac_handle_asoc_rsp_sta(hmac_vap_stru *pst_hmac_vap, oal_uint8 *puc_msg)
{
    ...
    else
    {
        ...
        {
#ifdef _PRE_WLAN_1103_CHR
            hmac_chr_set_connect_code(pst_hmac_vap->st_vap_base_info.uc_vap_id, pst_asoc_rsp->en_status_code);
#endif
