...
oal_void hmac_chan_update_40M_intol_user(mac_vap_stru *pst_mac_vap)
{
    ...
    OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &(pst_mac_vap->st_mac_user_list_head))
    {
        ...
        {
            OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_2040, "{hmac_chan_update_40M_intol_user::pst_user null pointer.}");
            continue;
        }
        else
        {
...
OAL_STATIC oal_void  hmac_cac_chan_ctrl_machw_tx_event(mac_vap_stru *pst_mac_vap, oal_uint8 uc_cac_machw_en)
{
    ...
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_SCAN, "{hmac_cac_chan_ctrl_machw_tx::frw_event_dispatch_event failed[%d].}", ul_ret);
        FRW_EVENT_FREE(pst_event_mem);
        return;
    }
...
oal_uint32  hmac_chan_start_bss(hmac_vap_stru *pst_hmac_vap, mac_channel_stru *pst_channel, wlan_protocol_enum_uint8 en_protocol)
{
    ...
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN, "{hmac_chan_start_bss::hmac_sta_sync_vap failed[%d].}", ul_ret);
        return ul_ret;
    }
...
oal_uint32  hmac_chan_switch_to_new_chan_complete(frw_event_mem_stru *pst_event_mem)
{
    ...
    if(IS_STA(pst_mac_vap))
    {
        ...
        if(OAL_SUCC == ul_ret)
        {
            OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC, "{hmac_chan_switch_to_new_chan_complete::after hmac_check_ap_channel_follow_sta ap channel change from %d to %d}",
