...
oal_uint32 hmac_config_11i_add_key(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (OAL_TRUE == en_pairwise)
    {
        ...
        if (OAL_SUCC != ul_ret)
        {
            ...
            OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_WPA, "{hmac_config_11i_add_key::us_user_idx[%d] find_user_by_macaddr fail[%d].}", us_user_idx, ul_ret);
            return ul_ret;
        }
    }
    else
...
oal_uint32 hmac_config_11i_remove_key(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (OAL_PTR_NULL == pst_priv_key)
    {
        ...
        {
            OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_WPA, "{hmac_config_11i_remove_key::user already del.}");
            return OAL_SUCC;
        }
        else
        {
...
oal_uint32 hmac_11i_ether_type_filter(hmac_vap_stru *pst_vap, hmac_user_stru *pst_hmac_user, oal_uint16 us_ether_type)
{
