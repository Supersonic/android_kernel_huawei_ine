...
oal_uint32  hmac_dfs_cac_timeout_fn(oal_void *p_arg)
{
    ...
    ul_ret = mac_get_channel_idx_from_num(MAC_RC_START_FREQ_5,pst_mac_device->uc_max_channel,&uc_idx);
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(0, OAM_SF_DFS, "{[DFS]hmac_dfs_cac_timeout_fn::mac_get_channel_idx_from_num failed=%d}",ul_ret);
    }
    st_cac_event.en_type = HMAC_CAC_FINISHED;
    st_cac_event.ul_freq = g_ast_freq_map_5g[uc_idx].us_freq;
    ...
    if(OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(0, OAM_SF_DFS, "{[DFS]hmac_dfs_cac_timeout_fn::hmac_dfs_cac_event_report failed=%d}",ul_ret);
    }
...
OAL_STATIC oal_uint32  hmac_chan_get_cac_time(mac_device_stru *pst_mac_device, mac_vap_stru *pst_mac_vap)
{
    ...
    {
        ul_ret = mac_get_channel_idx_from_num(pst_mac_vap->st_channel.en_band, pst_mac_vap->st_channel.uc_chan_number, &uc_idx);
        if (OAL_SUCC != ul_ret)
        {
            OAM_ERROR_LOG2(0, OAM_SF_DFS, "{hmac_chan_get_cac_time::mac_get_channel_idx_from_num fail. en_band:%d, offchan_num:%u}",
...
OAL_STATIC oal_uint32  hmac_chan_get_off_chan_cac_time(mac_device_stru *pst_mac_device, mac_vap_stru *pst_mac_vap)
{
    ...
    mac_get_channel_idx_from_num(pst_mac_vap->st_channel.en_band, pst_mac_device->st_dfs.st_dfs_info.uc_offchan_num, &uc_idx);
...
oal_void  hmac_dfs_cac_stop(mac_device_stru *pst_mac_device,mac_vap_stru *pst_mac_vap)
{
    oal_uint32  ul_ret;
    oal_uint8   uc_idx = 0;
    hmac_cac_event_stru   st_cac_event;
    ...
    if (OAL_TRUE == pst_mac_device->st_dfs.st_dfs_cac_timer.en_is_enabled)
    {
        ...
        if(OAL_SUCC != ul_ret)
        {
            OAM_WARNING_LOG1(0, OAM_SF_DFS, "{[DFS]hmac_dfs_cac_stop::hmac_dfs_cac_event_report failed=%d}",ul_ret);
        }
...
oal_void  hmac_dfs_cac_start(mac_device_stru *pst_mac_device, hmac_vap_stru *pst_hmac_vap)
{
    ...
    if(OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(0, OAM_SF_DFS, "{[DFS]hmac_dfs_cac_start::hmac_dfs_cac_event_report failed=%d}",ul_ret);
    }
...
oal_void  hmac_dfs_radar_wait(mac_device_stru *pst_mac_device, hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (0 == uc_5g_vap_cnt)
    {
        OAM_WARNING_LOG1(0, OAM_SF_DFS, "{hmac_dfs_radar_wait::reset dfs status for device[%d] after all 5g vap down", pst_mac_device->uc_device_id);
    ...
    if(pst_mac_device->st_dfs.st_dfs_info.uc_offchan_flag == (BIT0|BIT1))
    {
        ...
        if (OAL_SUCC != ul_ret)
        {
            OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_DFS,
                           "{hmac_dfs_channel_mark_radar::mac_get_channel_idx_from_num failed = %d.}", ul_ret);
            return OAL_FAIL;
        }
                ...
                wlan_channel_bandwidth_enum_uint8   en_bandwidth)
{
    ...
    ul_ret = mac_get_channel_idx_from_num(pst_mac_vap->st_channel.en_band,uc_channel,&uc_target_idx);
    if(ul_ret != OAL_SUCC)
    {
         OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_DFS,
...
oal_uint32  hmac_dfs_ap_wait_start_radar_handler(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if(pst_mac_device->st_dfs.st_dfs_info.uc_custom_next_chnum)
    {
        ...
        OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_DFS,
                         "{hmac_dfs_ap_wait_start_radar_handler::next ch(%d) and bw(%d) set by user.}",
                         uc_new_channel,en_new_bandwidth);

        if(hmac_dfs_is_channel_support_bw(pst_mac_vap, uc_new_channel, en_new_bandwidth) != OAL_SUCC)
        {
            OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_DFS,
                    "{hmac_dfs_switch_channel_for_radar::new channel(%d) not support current bw mode(%d).}",uc_new_channel,en_new_bandwidth);
...
oal_uint32 hmac_dfs_switch_channel_for_radar(mac_device_stru *pst_mac_device, mac_vap_stru *pst_mac_vap)
{
    ...
    if(pst_mac_device->st_dfs.st_dfs_info.uc_custom_next_chnum)
    {
        ...
        OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_CHAN,
                         "{hmac_dfs_switch_channel_for_radar::next ch(%d) and bw(%d) set by user.}",
                         uc_new_channel,en_new_bandwidth);
...
oal_bool_enum_uint8 hmac_dfs_try_cac(hmac_device_stru *pst_hmac_device, mac_vap_stru *pst_mac_vap)
{
    ...
    {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id,OAM_SF_ANY,"{hmac_dfs_try_cac::mac_res_get_hmac_vap fail.vap_id[%u]}",pst_mac_vap->uc_vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (IS_LEGACY_VAP(&(pst_hmac_vap->st_vap_base_info)))
    {
        ...
        {
            // 此时设置硬件信道
            hmac_chan_sync(&pst_hmac_vap->st_vap_base_info, pst_channel->uc_chan_number,
