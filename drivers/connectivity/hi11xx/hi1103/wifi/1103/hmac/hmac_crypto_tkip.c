...
oal_uint32 hmac_crypto_tkip_enmic_etc(wlan_priv_key_param_stru *pst_key, oal_netbuf_stru *pst_netbuf)
{
    ...
    /*4.1 计算mic */
	ul_ret = hmac_crypto_tkip_michael_mic(puc_tx_mic_key, pst_netbuf, MAC_GET_CB_FRAME_HEADER_ADDR(pst_cb), ul_hdrlen, ul_pktlen - ul_hdrlen, auc_mic);
    if (OAL_SUCC != ul_ret)
    {
        return ul_ret;
    }
...
oal_uint32 hmac_crypto_tkip_demic_etc(wlan_priv_key_param_stru *pst_key, oal_netbuf_stru *pst_netbuf)
{
    ...
    /*5.1 计算mic */
	ul_ret = hmac_crypto_tkip_michael_mic(puc_rx_mic_key, pst_netbuf, (mac_ieee80211_frame_stru *)MAC_GET_RX_CB_MAC_HEADER_ADDR(pst_cb), ul_hdrlen, ul_pktlen - (ul_hdrlen + IEEE80211_WEP_MICLEN), auc_mic);
    if (OAL_SUCC != ul_ret)
    {
        return ul_ret;
    }
