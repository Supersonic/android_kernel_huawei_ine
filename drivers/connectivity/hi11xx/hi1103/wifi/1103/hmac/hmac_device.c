...
OAL_STATIC oal_uint32  hmac_chip_init(mac_chip_stru *pst_chip)
{
    ...
    //mac_chip_init(pst_dst_chip, pst_src_chip->uc_device_nums);
...
oal_uint32  hmac_board_init(oal_uint32 ul_chip_max_num)
{
    oal_uint8              uc_chip;
    oal_uint32             ul_ret;
    ...
    for (uc_chip = 0; uc_chip < ul_chip_max_num; uc_chip++)
    {
        ...
        if (OAL_SUCC != ul_ret)
        {
             OAM_WARNING_LOG2(0, OAM_SF_ANY, "{hmac_board_init::hmac_chip_init failed[%d], uc_chip_id[%d].}", ul_ret, uc_chip);
             return OAL_FAIL;
        }
        /* dmac执行 */
