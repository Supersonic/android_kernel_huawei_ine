...
oal_uint32  hmac_chr_get_web_fail_slow_info(oal_void)
{
    ...
    if (OAL_PTR_NULL == pst_mac_device)
    {
        OAM_ERROR_LOG0(0, OAM_SF_ACS, "{hmac_chr_get_web_fail_slow_info::pst_mac_device null.}");
        return OAL_FALSE;
    }
    ...
    for (uc_vap_index = 0; uc_vap_index < pst_mac_device->uc_vap_num; uc_vap_index++)
    {
        ...
        if(IS_LEGACY_STA(pst_mac_vap))
        {
            ...
            CHR_EXCEPTION_P(CHR_WIFI_WEB_FAIL_EVENTID, (oal_uint8 *)(&st_disasoc_reason), OAL_SIZEOF(hmac_chr_disasoc_reason_stru));
        }
    }
...
oal_uint32  hmac_chr_get_connect_info(oal_void)
{
    ...
    for (uc_vap_index = 0; uc_vap_index < pst_mac_device->uc_vap_num; uc_vap_index++)
    {
        ...
        if(IS_LEGACY_STA(pst_mac_vap))
        {
            ...
            {
                CHR_EXCEPTION_P(CHR_WIFI_CONNECT_FAIL_EVENTID, (oal_uint8 *)pst_connect_code, OAL_SIZEOF(oal_uint16));
            }
        }
    }
...
oal_uint32  hmac_get_chr_info_event_hander(oal_uint32 chr_event_id)
{
    ...
    switch (chr_event_id)
    {
        case CHR_WIFI_WEB_FAIL_EVENTID:
            ul_ret = hmac_chr_get_web_fail_slow_info();
            if (ul_ret != OAL_SUCC)
            {
                OAM_WARNING_LOG0(0, OAM_SF_ANY, "{hmac_get_chr_info_event_hander::hmac_chr_get_web_fail_slow_info fail.}");
                return ul_ret;
            }

            break;

        case CHR_WIFI_CONNECT_FAIL_EVENTID:
            ul_ret = hmac_chr_get_connect_info();
            if (ul_ret != OAL_SUCC)
            {
                OAM_WARNING_LOG0(0, OAM_SF_ANY, "{hmac_get_chr_info_event_hander::hmac_chr_get_connect_info fail.}");
                return ul_ret;
            }
