                ...
                mac_action_mgmt_args_stru  *pst_action_args)
{
    ...
    /* 对tid对应的txBA会话状态加锁 */
    oal_spin_lock(&(pst_hmac_user->ast_tid_info[uc_tidno].st_ba_tx_info.st_ba_status_lock));
    ...
    if (OAL_PTR_NULL == pst_event_mem)
    {
        ...
        oal_spin_unlock(&(pst_hmac_user->ast_tid_info[uc_tidno].st_ba_tx_info.st_ba_status_lock));
        return OAL_ERR_CODE_PTR_NULL;
    }
...
#ifdef _PRE_WLAN_1103_CHR
    hmac_chr_set_ba_info(uc_tidno, (oal_uint16)pst_action_args->ul_arg3);
#endif
    /* 初始化CB */
                ...
                oal_uint8        *puc_payload)
{
    ...
    /* 对tid对应的tx BA会话状态加锁 */
    oal_spin_lock(&(pst_tid->st_ba_tx_info.st_ba_status_lock));
    ...
    if (MAC_RECIPIENT_DELBA == uc_initiator)
    {
        if (DMAC_BA_INIT == pst_tid->st_ba_tx_info.en_ba_status)
        {
            oal_spin_unlock(&(pst_tid->st_ba_tx_info.st_ba_status_lock));
            return OAL_SUCC;
        }
