...
oal_uint16 hmac_tcp_opt_rx_tcp_ack_filter(void *hmac_vap, hmac_tcp_opt_queue type,hcc_chan_type dir, void* data)
{
   ...
   if(OAL_WARN_ON(!data))
    ...
    {
        OAM_INFO_LOG2(0,OAM_SF_ANY,"{hmac_tcp_opt_rx_tcp_ack_filter fail:type:%d not equal %d}",type,HMAC_TCP_ACK_QUEUE);
        return OAL_FAIL;
    } 
...
oal_uint16 hmac_tcp_opt_tx_tcp_ack_filter(void *hmac_device, hmac_tcp_opt_queue type,hcc_chan_type dir, void* data)
{
    ...
    if(OAL_WARN_ON(!hmac_device))
    {
        OAM_INFO_LOG0(0,OAM_SF_ANY,"{hmac_tcp_opt_rx_tcp_ack_filter fail:hmac_device is null}");
        return OAL_FAIL;
    } 
...
oal_int32 hmac_tcp_ack_process(void)
{
    ...
    {
        pst_hmac_device = hmac_res_get_mac_dev(pst_mac_chip->auc_device_id[uc_device]);//多device循环chip下的所有device
        if (OAL_PTR_NULL == pst_hmac_device)
        {
