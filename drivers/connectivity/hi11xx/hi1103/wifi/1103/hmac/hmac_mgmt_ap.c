...
OAL_STATIC oal_uint16 hmac_check_rsn_ap(mac_vap_stru *pst_mac_vap, mac_user_stru *pst_mac_user, oal_uint8 *puc_payload, oal_uint32 ul_msg_len)
{
    ...
    /* 获取RSN和WPA IE信息 */
    puc_rsn_ie = mac_find_ie(MAC_EID_RSN, puc_payload, (oal_int32)ul_msg_len);
    puc_wpa_ie = mac_find_vendor_ie(MAC_WLAN_OUI_MICROSOFT, MAC_OUITYPE_WPA, puc_payload, (oal_int32)ul_msg_len);
            ...
            oal_uint8 *puc_payload, oal_uint32 ul_msg_len)
{
    ...
    puc_ie = mac_find_ie(MAC_EID_RRM, puc_payload, (oal_int32)ul_msg_len);
                ...
                mac_status_code_enum_uint16    *pen_status_code)
{
    ...
    ***************************************************************************/
    ...
    puc_ie_tmp = mac_find_ie(MAC_EID_SSID, puc_payload, (oal_int32)ul_msg_len);
    ...
    puc_ie_tmp = mac_find_ie(MAC_EID_WMM, puc_payload, (oal_int32)ul_msg_len);
    if (OAL_PTR_NULL != puc_ie_tmp)
    {
    ...
    /* 更新QoS能力 */
    hmac_mgmt_update_assoc_user_qos_table(puc_payload, (oal_uint16)ul_msg_len, pst_hmac_user);

#ifdef _PRE_WLAN_FEATURE_TXBF
    /* 更新11n txbf能力 */
    puc_ie_tmp = mac_find_vendor_ie(MAC_HUAWEI_VENDER_IE, MAC_EID_11NTXBF, puc_payload, (oal_int32)ul_msg_len);
    hmac_mgmt_update_11ntxbf_cap(puc_ie_tmp, pst_hmac_user);
#endif

    /* 更新11ac VHT capabilities ie */
    puc_ie_tmp = mac_find_ie(MAC_EID_VHT_CAP, puc_payload, (oal_int32)ul_msg_len);
    if (OAL_PTR_NULL != puc_ie_tmp)
    {
    ...
    if (pst_hmac_user->st_user_base_info.st_vht_hdl.en_vht_capable == OAL_FALSE)
    {
        ...
        puc_vendor_vht_ie = mac_find_vendor_ie(MAC_WLAN_OUI_BROADCOM_EPIGRAM, MAC_WLAN_OUI_VENDOR_VHT_TYPE, puc_payload, ul_msg_len);
...
#if (_PRE_TARGET_PRODUCT_TYPE_ONT == _PRE_CONFIG_TARGET_PRODUCT)
    st_huawei_ie_info.puc_ie_buf = mac_find_vendor_ie(MAC_WLAN_OUI_HUAWEI, MAC_WLAN_OUI_TYPE_HUAWEI_CASCADE, puc_payload , ul_payload_len );
...
//#endif
    ...
    if (MAC_REJECT_TEMP != en_status_code)
    {
        ...
        puc_ie_tmp = mac_find_ie(MAC_EID_MOBILITY_DOMAIN, puc_payload_tmp, l_payload_len_tmp);
    ...
    if ( pst_hmac_vap->st_wds_table.en_wds_vap_mode == WDS_MODE_ROOTAP )
    {
        ...
        else
        {
            ...
            if (OAL_PTR_NULL != mac_find_vendor_ie(MAC_WLAN_OUI_HUAWEI, MAC_WLAN_OUI_TYPE_HAUWEI_4ADDR, puc_payload + l_not_ie_len, (oal_int32)ul_payload_len - l_not_ie_len))
            {
                pst_hmac_user->uc_is_wds = OAL_TRUE;
                OAM_WARNING_LOG0(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_ASSOC, "{hmac_ap_up_rx_asoc_req::user surpport 4 address.}");
            }
