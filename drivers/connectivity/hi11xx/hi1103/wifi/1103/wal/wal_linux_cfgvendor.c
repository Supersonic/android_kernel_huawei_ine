                                                          ...
                                                          oal_wireless_dev_stru *wdev, OAL_CONST oal_void  *data, oal_int32 len)
{
    ...
    skb = oal_cfg80211_vendor_cmd_alloc_reply_skb(wiphy, sizeof(gscan_capa));
    if (OAL_UNLIKELY(!skb))
    {
    ...
    ul_ret = oal_cfg80211_vendor_cmd_reply(skb);
    if (OAL_UNLIKELY(ul_ret))
    {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "wal_cfgvendor_get_gscan_capabilities::oal_cfg80211_vendor_cmd_reply failed ret:%d.\r\n", ul_ret);
        return ul_ret;
    }
    ...
    oal_wireless_dev_stru *wdev, OAL_CONST oal_void  *data, oal_int32 len)
{
    ...
    if (OAL_UNLIKELY(err))
    {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "wal_cfgvendor_get_channel_list::oal_cfg80211_vendor_cmd_reply failed ret:%d.\r\n", err);
        return err;
    }
                                                                 ...
                                                                 oal_int32 l_len)
{
    ...
    if(uc_ip_pkt_len > 0 && OAL_PTR_NULL != puc_ip_pkt)
    {
       ...
       #if 1/*待正式测试完成后，删除*/
       OAM_ERROR_LOG4(0, OAM_SF_ANY, "{wl_cfgvendor_start_vowifi_nat_keep_alive::en_type=[%d],id=[%d],ip_len=[%d],period=[%d].}",
        ...
        switch (l_type) {
            ...
            case APF_ATTRIBUTE_PROGRAM:
                ...
                ***************************************************************************/
                ...
                if (OAL_UNLIKELY(OAL_SUCC != l_ret))
                {
                    OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_cfgvendor_apf_set_filter::send apf filter fail, err code %d!}\r\n", l_ret);
                    return l_ret;
                }
            break;
            default:
