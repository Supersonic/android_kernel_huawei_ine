...
void bfgx_beat_timer_expire(uint64 data)
{
    ...
    if (BFGX_NOT_RECV_BEAT_INFO == atomic_read(&pst_exception_data->bfgx_beat_flag))
    {
        ...
        if (PLAT_EXCEPTION_ENABLE == g_plat_beatTimer_timeOut_reset_cfg)
        {
            ...
            plat_exception_handler(SUBSYS_BFGX, BFGX_THREAD_BOTTOM, BFGX_BEATHEART_TIMEOUT);
...
int32 plat_get_dfr_sinfo(char* buf,int32 index)
{
    ...
    i += snprintf( buf + i, PAGE_SIZE - i,"==========dfr info:=========\n");
    i += snprintf( buf + i, PAGE_SIZE - i,"total cnt:%-10d\n",plat_get_excp_total_cnt());
    for(etype = 0;etype < EXCEPTION_TYPE_BOTTOM ;etype++)
    {
        i += snprintf( buf + i, PAGE_SIZE - i,"id:%-30scnt:%-10dfail:%-10dsingle_sysrst:%-10dall_sysrst:%-10dmaxtime:%lldms\n", excp_info_str_get(etype),
...
int32 bfgx_exception_handler(void)
{
    ...
    else
    {
        ...
        if (BFGX_LASTWORD_PANIC == exception_type)
        {
