...
static bool s_imx576_power_on = false; //false for power down, ture for power up
...
static int imx576_config_power_on(hwsensor_intf_t* si)
{
    ...
    if (!s_imx576_power_on){
        ret = si->vtbl->power_up(si);
        if (0 == ret) {
            s_imx576_power_on = true;
        } else {
