...
static bool s_hi259b_power_on = false; //false for power down, ture for power up
struct mutex hi259b_power_lock;
static hwsensor_intf_t *s_intf = NULL;
static sensor_t *s_sensor = NULL;
static sensor_t s_hi259b;
static struct sensor_power_setting hi259b_power_setting[] = {
    //SEL=1 SWITCH MIPI to M0, power off output low to avoid switch leakage current
    {
        .seq_type = SENSOR_MIPI_SW,
        .config_val = SENSOR_GPIO_HIGH, //out down
        .sensor_index = SENSOR_INDEX_INVALID,
        .delay = DELAY_0MS,
    },

    //disable back camera M0 reset
    {
    ...
    //S1+S0+M0+M1 IOVDD&DVDD 1.80V[LDO21]
    {
...
static struct sensor_power_setting hi259b_power_down_setting[] = {
...
static int hi259b_csi_enable(hwsensor_intf_t* si)
{
    return 0;
}

static int hi259b_csi_disable(hwsensor_intf_t* si)
{
    return 0;
}
...
static int hi259b_config_power_on(hwsensor_intf_t* si)
{
    int ret = 0;
    ...
    if (!s_hi259b_power_on){
        ret = si->vtbl->power_up(si);
        if (0 == ret) {
            s_hi259b_power_on = true;
        } else {
...
static int hi259b_config_power_off(hwsensor_intf_t* si)
{
    int ret = 0;
    mutex_lock(&hi259b_power_lock);
...
static int hi259b_config_match_id(hwsensor_intf_t* si, void *argp)
{
    int ret = 0;
        ...
        void  *argp)
{
    ...
    switch(data->cfgtype){
        case SEN_CONFIG_POWER_ON:
            ret = hi259b_config_power_on(si);
            break;
        case SEN_CONFIG_POWER_OFF:
            ret = hi259b_config_power_off(si);
            break;
        ...
        case SEN_CONFIG_MATCH_ID:
            ret = hi259b_config_match_id(si, argp);
            break;
...
static sensor_t s_hi259b =
{
    ...
    .power_setting_array = {
        .size = ARRAY_SIZE(hi259b_power_setting),
        .power_setting = hi259b_power_setting,
    },
...
static int32_t hi259b_platform_remove(struct platform_device* pdev);
static struct platform_driver
s_hi259b_driver =
{
    .probe = hi259b_platform_probe,
    .remove = hi259b_platform_remove,
    .driver =
    {
        .name = "huawei,hi259b",
        .owner = THIS_MODULE,
