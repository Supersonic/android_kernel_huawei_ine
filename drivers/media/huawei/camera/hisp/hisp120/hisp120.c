...
struct rpmsg_service_info {
    struct rpmsg_hisp120_service *hisi_isp_serv;
...
void hisp120_handle_msg(hisp_msg_t *msg)
{
    ...
    switch (msg->api_name)
    {
        case REQUEST_RESPONSE:
            hisp120_set_timestamp(&(msg->u.ack_request.timestampH), &(msg->u.ack_request.timestampL));
            break;
        case MSG_EVENT_SENT:
            hisp120_set_timestamp(&(msg->u.event_sent.timestampH), &(msg->u.event_sent.timestampL));
            break;
...
{
    struct rpmsg_hisp120_service *hisi_serv = rpmsg_local.hisi_isp_serv;
...
#if 0
static void
hisp120_power_req(hisp_msg_t *msg, int len)
{
    struct rpmsg_hisp120_service *hisi_serv = rpmsg_local.hisi_isp_serv;
...
static void
hisp120_rpmsg_ept_cb(struct rpmsg_channel *rpdev,
...
static int hisp120_get_a7isp_addr(void *cfg)
{
    ...
    if (IS_ERR_OR_NULL(s_hisp120.ion_client)) {
        cam_err("func %s: s_hisp120.ion_client is NULL or ERR",__func__);
        goto err_ion_client;
    }

    hdl = ion_import_dma_buf(s_hisp120.ion_client, pcfg->param.sharedFd);
    ...
    table = ion_sg_table(s_hisp120.ion_client, hdl);
    ...
    ion_free(s_hisp120.ion_client, hdl);
...
static int hisp120_init_r8isp_memory_pool(void *cfg)
{
    ...
    if(pcfg->param.type == MAP_TYPE_RAW2YUV){
        ...
        {
            if (s_hisp120.mem_pool[ipool].prot == pcfg->param.prot) {
                break;
            }
        }
    ...
    s_hisp120.mem_pool[ipool].security_isp_mode  = pcfg->param.security_isp_mode;
    ...
    if(ipool == MEM_POOL_ATTR_READ_WRITE_CACHE_OFF_LINE)
    {
        ...
        snprintf(ion_name, sizeof(ion_name), "hwcam-hisp120-%d", ipool);
        s_hisp120.mem_pool[ipool].ion_client = hisi_ion_client_create(ion_name);
        if (IS_ERR_OR_NULL(s_hisp120.mem_pool[ipool].ion_client  )) {
            cam_err("failed to create ion client %s\n", ion_name);
            goto err_ion_client;
        }
    }

    ion_client = s_hisp120.mem_pool[ipool].ion_client;
    ...
    if(ipool == MEM_POOL_ATTR_READ_WRITE_CACHE_OFF_LINE){
        s_hisp120.mem_pool[ipool].r8_iova  = hisp_mem_map_steup(sg, pcfg->param.iova,
            pcfg->param.size,
            pcfg->param.prot,
            (unsigned int)ipool,
            MAP_TYPE_RAW2YUV,
    ...
    if (!s_hisp120.mem_pool[ipool].r8_iova) {
    ...
    cam_info("func %s: r8_iova_pool_base=0x%x",__func__, s_hisp120.mem_pool[ipool].r8_iova);
...
static int hisp120_deinit_r8isp_memory_pool(void *cfg)
{
    ...
    if(s_hisp120.mem_pool[ipool].ion_client
        && (ipool == MEM_POOL_ATTR_READ_WRITE_CACHE_OFF_LINE))
    {
        cam_debug("%s deinit memory pool ion client %d\n", __func__, ipool);
        ion_client_destroy(s_hisp120.mem_pool[ipool].ion_client);
        s_hisp120.mem_pool[ipool].ion_client = NULL;
    }
...
static int hisp120_alloc_r8isp_addr(void *cfg)
{
    ...
    pcfg->param.vaddr = (void *)(((unsigned char *)s_hisp120.mem_pool[ipool].ap_va) + offset);
...
static int hisp120_mem_pool_pre_init(void)
{
    ...
    for (ipool = 0; ipool < MEM_POOL_ATTR_MAX; ipool++)
    {
        memset(&(s_hisp120.mem_pool[ipool]), 0, sizeof(struct hisp120_mem_pool));
...
static int hisp120_config(hisp_intf_t *i, void *cfg)
{
    ...
    case HISP_CONFIG_POWER_ON:
        if (!remote_processor_up) {
            ...
            cam_notice("%s power on the hisp120.\n", __func__);
            ...
            cam_warn("%s begin to power on the hisp120.\n", __func__);
            rc = hisp120_power_on(i);
        }
        break;
    case HISP_CONFIG_POWER_OFF:
        if (remote_processor_up) {
            cam_notice("%s power off the hisp120.\n", __func__);
            rc = hisp120_power_off(i);
        }
        break;
...
static int hisp120_power_on(hisp_intf_t *i)
{
    ...
    struct rpmsg_hisp120_service *hisi_serv = NULL;
    if (NULL == i){
    ...
    s_hisp120.ion_client = hisi_ion_client_create("hwcam-hisp120");
    if (IS_ERR_OR_NULL(s_hisp120.ion_client )) {
...
FAILED_RET:
    ...
    if (ion_client_created) {
        ion_client_destroy(s_hisp120.ion_client);
...
static void hisp120_rpmsg_remove(struct rpmsg_channel *rpdev)
{
    struct rpmsg_hisp120_service *hisi_serv = dev_get_drvdata(&rpdev->dev);
...
static void
hisp120_rpmsg_driver_cb(struct rpmsg_channel *rpdev,
            void *data, int len, void *priv, u32 src)
{
    cam_info("%s enter ....\n", __func__);
...
static int
hisp120_send_rpmsg(hisp_intf_t *i, hisp_msg_t *from_user, size_t len)
{
    int rc = 0;
    hisp120_t *hi = NULL;
    struct rpmsg_hisp120_service *hisi_serv = NULL;
...
static int
hisp120_recv_rpmsg(hisp_intf_t *i, hisp_msg_t *user_addr, size_t len)
{
    int rc = len;
    hisp120_t *hi = NULL;
    struct rpmsg_hisp120_service *hisi_serv = NULL;
...
static int32_t hisp120_rpmsg_probe(struct rpmsg_channel *rpdev)
{
    int32_t ret = 0;
    struct rpmsg_hisp120_service *hisi_serv = NULL;
    cam_info("%s enter\n", __func__);
    ...
    struct platform_device* pdev)
{
    ...
    ret = hisp_register(&s_hisp120.intf, &s_hisp120.notify);
    if (0 == ret) {
        atomic_set(&s_hisp120.opened, 0); /*lint !e1058 */
    } else {
    ...
    if (ret < 0) {
        ...
        hisp_unregister(&s_hisp120.intf);
        goto error;
    }
#endif
    return 0;
