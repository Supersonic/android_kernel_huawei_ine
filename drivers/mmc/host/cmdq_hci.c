...
#endif
{
	...
	if (err_info & CQTERRI_RES_ERR)
		...
		pr_err("no cmd or data task index get!\n");

	/*
	 * In some case, the tag index is not in CQTDBR,
	 * so mrq is null if we get it from tag, use the
	 * first dbr set bit as the tag index.
	 */
	if ((tag == 0xFFFF) || ((tag != 0xFFFF) && (!(dbr_set & (1 << tag))))) {
#ifdef CONFIG_EMMC_FAULT_INJECT
		if (do_inj) {
	...
	if(!mrq) {
		pr_err("get invalid null mrq!\n");
		BUG_ON(1);
	}

	if(err_info & CQTERRI_DAT_ERR) {
		if (mrq->data)
			mrq->data->error = err;
	} else {
		if (mrq->cmd)
