...
#define CVT_MAX_SAVE_NUM                200
#define CVT_VALID_CYCLE                 2000 /* PERCENT cycles */
#define CVT_PARA_CHECK_TIMES            5
#define CVT_VALID_VOLT_MV               4250
#define CVT_GETCUR_GAPTIME              200 /* 200ms */
#define CVT_T1_CURR_HIGH                505
#define CVT_T1_CURR_LOW                 495
#define CVT_T1_RECOUP_CURR              485
#define CVT_T2_CURR_LOW                 195
#define CVT_T2_CURR_HIGH                205
#define CVT_T2_RECOUP_CURR              185
#define CVT_DATA_DIRECTORY              "/splash2/battery"
#define CVT_DATA_FILE                   "/splash2/battery/cvt.data"
#define CVT_FILE_MAGIC_NUM              0x3c5a7711
#define CVT_RECOUP_RATIO_COUNT          8
#define CVT_WORK_INTERVAL_MS            4000 /* work 5s and sleep 1s */
#define CVT_CURR_RECOUP_S               2
#define CVT_PROCESS_START_SOC           90
#define CVT_RECORD_TEMP_MAX             450 /* temp in centigrade and TENTH */
#define CVT_RECORD_TEMP_MIN             100 /* temp in centigrade and TENTH */
#define CVT_INTER_TIME_MS               1000
#define CVT_SPLASH2_INIT_RETRY          3
#define CVT_SPLASH2_INIT_OK             4

enum cvt_recoup_type {
    CVT_RECOUP_TEMP = 0,
    CVT_RECOUP_RATIO,
    CVT_RECOUP_RATIO_TYPE,
};

struct coul_cvt_info {
    unsigned char cnt_start;
    unsigned char cnt_end;
    unsigned short cvt_retry;
    unsigned short adj_cvt;
    unsigned int org_cvt;
    unsigned int recoup_T1;
    unsigned int recoup_T2;
    unsigned int cvt_start_time;
    unsigned int cvt_end_time;
    int recoup_temp[CVT_RECOUP_RATIO_COUNT];
    int recoup_ratio[CVT_RECOUP_RATIO_COUNT];
    struct delayed_work cvt_delayed_work;
};

struct coul_cvt_file {
    unsigned char cvt_status;
    unsigned char dmd_report;
    unsigned short valid_num;
    unsigned int magic_num;
    char batt_brand[BATT_BRAND_STRING_MAX];
    unsigned short org_cvt[CVT_MAX_SAVE_NUM]; /* original cvt */
    unsigned short adj_cvt[CVT_MAX_SAVE_NUM]; /* adjust cvt */
    unsigned short batt_cycle[CVT_MAX_SAVE_NUM];
    unsigned short batt_temp[CVT_MAX_SAVE_NUM];
    time_t cvt_time[CVT_MAX_SAVE_NUM]; /* long int */
};
...
struct smartstar_coul_device
{
	...
	unsigned int ocv_invalid_for_fast_fcc;
	unsigned int fast_fcc_enable;
	int system_ready;
    ...
    struct device *dev;
    struct coul_cvt_info *cvt_info;
    struct coul_cvt_file cvt_file;
    unsigned int cvt_support;
    int is_nv_read;
    int is_nv_need_save;
    int last_soc_enable;
