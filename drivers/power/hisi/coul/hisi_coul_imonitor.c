...
static int coul_imonitor_cvt_prepare_basic_info(
...
static struct coul_imonitor_event_id imonitor_event_id[] = {
	...
	{
		COUL_IMONITOR_CVT_BASIC_INFO,
		COUL_IMONITOR_CVT_BASIC_INFO_NO,
		.prepare_cb = coul_imonitor_cvt_prepare_basic_info,
		{.tv_sec = 0, .tv_nsec = 0},
		COUL_IMONITOR_REPORT_INTERVAL_HOUR
	},
};
	...
	struct imonitor_eventobj *obj, void *data)
{
	...
	cvt = (struct coul_imonitor_cvt_basic_info *)data;

	ret = imonitor_set_param_integer_v2(obj, "OrgCvt", cvt->org_cvt);
	ret += imonitor_set_param_integer_v2(obj, "CalCvt", cvt->adj_cvt);
	ret += imonitor_set_param_integer_v2(obj, "BatCyc", cvt->batt_cycle);
	ret += imonitor_set_param_integer_v2(obj, "BatTemp", cvt->batt_temp);
	ret += imonitor_set_param_string_v2(obj, "BatVendor", cvt->batt_brand);
	ret += imonitor_set_param_string_v2(obj, "DetTime", cvt->systime);
	coul_imonitor_info("%s ret = %d\n", __func__, ret);
	return ret;
}
