...
#define MAX_TMP_BUF_LEN (10)
    ...
    while (*begin != '\0' && *begin != '\n')
    {
		...
		memset_s(num, sizeof(num), 0, sizeof(num));
		begin = end;
	}
...
static int basp_nv_get(char *buffer, const struct kernel_param *kp)
{
	...
	for (i = 0; i < MAX_RECORDS_CNT; i++) {
		snprintf_s(buffer + len, PAGE_SIZE - len, PAGE_SIZE - len -1,
			"learned_fcc[%d]:%d\n",
			i, di->nv_info.real_fcc_record[i]);
		len = strlen(buffer);
	}

	snprintf_s(buffer + len, PAGE_SIZE - len,
		PAGE_SIZE - len -1,
		"chargecycles:%d\n",
...
static BASP_LEVEL_TYPE find_basp_level(void)
{
	...
	for (idx = 0; idx < di->basp_total_level; idx++) {
...
static BASP_LEVEL_TYPE get_basp_level(struct smartstar_coul_device *di)
{
	...
	if (basp_mask_delay && (!di->basp_batt_matched ||
		di->basp_need_delay)) {
...
static void ocv_fcc_report(struct smartstar_coul_device *di)
{
	int ret;
	int pc;
	char timestamp[TIMESTAMP_STR_SIZE] = {0};
	char event_info[OCV_FCC_INFO_SIZE] = {0};
	static int last_ocv_mv = 0;

	if (last_ocv_mv == di->batt_ocv || !di->system_ready) {
		return;
	}
	last_ocv_mv = di->batt_ocv;
	if (di->ocv_invalid_for_fast_fcc) {
		coul_core_info("%s: ocv_invalid", __func__);
		di->ocv_invalid_for_fast_fcc = 0;
		return;
	}
	pc = calculate_pc(di, di->batt_ocv, di->batt_ocv_temp, di->batt_chargecycles);
	get_timestamp(timestamp, TIMESTAMP_STR_SIZE);
	ret = snprintf_s(event_info, OCV_FCC_INFO_SIZE, OCV_FCC_INFO_SIZE - 1, "%s %d %d %d %d %lld %d\n",
		timestamp, di->batt_ocv_temp / 10, di->batt_ocv / 1000, di->last_ocv_level,
		pc, di->iscd->full_update_cc, di->batt_chargecycles / 10);
	if (ret <= 0) {
		coul_core_info("snprintf_s failed\n");
		return;
	}
	coul_core_info("%s: %s \n", __func__, event_info);
	power_send_event_to_user("OCV_FCC_INFO", event_info);
}
#else
static void ocv_fcc_report(struct smartstar_coul_device *di)
{
...
/**********************************************************
*  Function:       cvt_info_clear
*  Description:    clear histroy record data
*  Parameters:     di:smartstar_coul_device
*  return value:   void
**********************************************************/
static void cvt_info_clear(struct smartstar_coul_device *di)
{
    di->cvt_info->cnt_start = 0;
    di->cvt_info->cnt_end = 0;
    di->cvt_info->recoup_T1 = 0;
    di->cvt_info->recoup_T2 = 0;
    di->cvt_info->cvt_start_time = 0;
    di->cvt_info->cvt_end_time = 0;
}

/**********************************************************
*  Function:       cvt_info_writer_to_file
*  Description:    writer cvt.data
*  Parameters:     di:smartstar_coul_device
*  return value:   void
**********************************************************/
static void cvt_writer_to_file(struct smartstar_coul_device *di)
{
    struct file *fd = NULL;
    ssize_t write_size;

    fd = filp_open(CVT_DATA_FILE, O_WRONLY|O_TRUNC, 0);
    if (IS_ERR(fd)) {
        coul_core_err("cvt splash2 file cvt.data not ready\n");
        return;
    }
    di->cvt_file.magic_num = CVT_FILE_MAGIC_NUM;
    write_size = kernel_write(fd, (const void *)&di->cvt_file,
        sizeof(di->cvt_file), 0);
    if (write_size != sizeof(di->cvt_file))
        coul_core_err("cvt Write %s failed, wtire: %zd, expect: %zu\n",
            CVT_DATA_FILE, write_size, sizeof(di->cvt_file));

    filp_close(fd, NULL);
    coul_core_info("sync cvt file to splash2 finished\n");
}

/**********************************************************
*  Function:       cvt_update_hist_info
*  Description:    update new data to cvt.data
*  Parameters:     di:charge_device_info
*  return value:   void
**********************************************************/
static void cvt_update_hist_info(struct smartstar_coul_device *di)
{
    int i;
    struct timespec ts;
    time_t time;
    unsigned short batt_cycles;
    unsigned short batt_temp;
    unsigned short org_cvt;
    ...
    if (!di->cvt_file.valid_num) {
        memcpy_s(di->cvt_file.batt_brand, sizeof(di->cvt_file.batt_brand),
            di->batt_data->batt_brand, BATT_BRAND_STRING_MAX);
    }

    ts = current_kernel_time();
    time = (time_t)(ts.tv_sec);
    batt_cycles = (unsigned short)coul_battery_cycle_count();
    batt_temp = (unsigned short)coul_get_battery_temperature();
    org_cvt = (unsigned short)di->cvt_info->org_cvt;

    if (di->cvt_file.valid_num < CVT_MAX_SAVE_NUM) {
        di->cvt_file.org_cvt[di->cvt_file.valid_num] = org_cvt;
        di->cvt_file.adj_cvt[di->cvt_file.valid_num] = di->cvt_info->adj_cvt;
        di->cvt_file.batt_cycle[di->cvt_file.valid_num] = batt_cycles;
        di->cvt_file.batt_temp[di->cvt_file.valid_num] = batt_temp;
        di->cvt_file.cvt_time[di->cvt_file.valid_num] = time;
        di->cvt_file.valid_num++;
    } else if (di->cvt_file.valid_num == CVT_MAX_SAVE_NUM) {
        for (i = 0; i < CVT_MAX_SAVE_NUM - 1; i++) {
            di->cvt_file.org_cvt[i] = di->cvt_file.org_cvt[i + 1];
            di->cvt_file.adj_cvt[i] = di->cvt_file.adj_cvt[i + 1];
            di->cvt_file.batt_cycle[i] = di->cvt_file.batt_cycle[i + 1];
            di->cvt_file.batt_temp[i] = di->cvt_file.batt_temp[i + 1];
            di->cvt_file.cvt_time[i] = di->cvt_file.cvt_time[i + 1];
        }
        di->cvt_file.org_cvt[i] = org_cvt;
        di->cvt_file.adj_cvt[i] = di->cvt_info->adj_cvt;
        di->cvt_file.batt_cycle[i] = batt_cycles;
        di->cvt_file.batt_temp[i] = batt_temp;
        di->cvt_file.cvt_time[i] = time;
    } else {
        di->cvt_file.valid_num = 1;
        di->cvt_file.org_cvt[0] = org_cvt;
        di->cvt_file.adj_cvt[0] = di->cvt_info->adj_cvt;
        di->cvt_file.batt_cycle[0] = batt_cycles;
        di->cvt_file.batt_temp[0] = batt_temp;
        di->cvt_file.cvt_time[0] = time;
    }
}

/**********************************************************
*  Function:       cvt_imonitor_record
*  Description:    report cvt imonitor
*  Parameters:     di:smartstar_coul_device
                   cvt:coul_imonitor_cvt_basic_info
*  return value:   void
**********************************************************/
static void cvt_imonitor_record(struct smartstar_coul_device *di,
                                struct coul_imonitor_cvt_basic_info *cvt)
{
    char timestamp[TIMESTAMP_STR_SIZE] = {0};

    if (di == NULL || cvt == NULL) {
        coul_core_err("cvt_imonitor_data di is null\n");
        return;
    }

    get_timestamp(timestamp, TIMESTAMP_STR_SIZE);
    memcpy_s(cvt->systime, sizeof(cvt->systime),
        timestamp, sizeof(char) * TIMESTAMP_STR_SIZE);
    memcpy_s(cvt->batt_brand, sizeof(cvt->batt_brand),
        di->batt_data->batt_brand, sizeof(char) * BATT_BRAND_STRING_MAX);
    cvt->adj_cvt = di->cvt_info->adj_cvt;
    cvt->org_cvt = (unsigned short)di->cvt_info->org_cvt;
    cvt->batt_temp = (unsigned short)coul_get_battery_temperature();
    cvt->batt_cycle = di->batt_chargecycles;
}

/**********************************************************
*  Function:       cvt_adjust
*  Description:    adjust cvt
*  Parameters:     di:smartstar_coul_device
*  return value:   void
**********************************************************/
static void cvt_adjust(struct smartstar_coul_device *di)
{
    int i;
    int coef = 1;

    if (di == NULL) {
        coul_core_err("cvt_adjust di is null\n");
        return;
    }

    for (i = 0; i < CVT_RECOUP_RATIO_COUNT - 1; i++) {
        if ((di->batt_temp >= di->cvt_info->recoup_temp[i]) &&
            (di->batt_temp <= di->cvt_info->recoup_temp[i + 1])) {
            coef = linear_interpolate(
                di->cvt_info->recoup_ratio[i],
                di->cvt_info->recoup_temp[i],
                di->cvt_info->recoup_ratio[i + 1],
                di->cvt_info->recoup_temp[i + 1],
                di->batt_temp);
            break;
        }
    }

    di->cvt_info->adj_cvt = (unsigned short)((di->cvt_info->org_cvt *
        CVT_INTER_TIME_MS) / coef);
    coul_core_info("cvt_adjust, adjust cvt = %d, coef = %d\n",
        di->cvt_info->adj_cvt, coef);
}

/**********************************************************
*  Function:       charge_batt_cvt_process
*  Description:    record cvt data
*  Parameters:     di:charge_device_info
*  return value:   void
**********************************************************/
static void charge_batt_cvt_process(struct smartstar_coul_device *di)
{
    int i;
    int batt_curr = 0;
    int batt_curr_sum = 0;
    ...
    if (di == NULL) {
        coul_core_err("charge_batt_cvt_process di is null\n");
        return;
    }

    if (di->charging_state != CHARGING_STATE_CHARGE_START) {
        coul_core_info("charge_batt_cvt_process, charge already stop\n");
        cvt_info_clear(di);
        return;
    }
    ...
    state = huawei_get_charge_state();
#endif
    if (((type == CHARGER_TYPE_STANDARD) || (type == CHARGER_TYPE_FCP) ||
        (type == CHARGER_TYPE_PD)) && (state == TRUE) &&
        (di->batt_chargecycles >= CVT_VALID_CYCLE) &&
        (!di->cvt_info->cnt_end)) {
        /* sleep 1000ms to calc accurate batt_curr */
        for (i = 0; i < CVT_PARA_CHECK_TIMES; i++) {
            batt_curr_sum += (-(coul_get_battery_current_ma()));
            batt_volt_sum += coul_get_battery_voltage_mv();
            msleep(CVT_GETCUR_GAPTIME);
        }
        batt_curr = batt_curr_sum / CVT_PARA_CHECK_TIMES;
        batt_volt = batt_volt_sum / CVT_PARA_CHECK_TIMES;
        coul_core_info("cvt process, batt_curr = %d, batt_volt = %d\n",
            batt_curr, batt_volt);
    } else {
        cvt_info_clear(di);
        coul_core_info("cvt process, type = %d, state = %d, cycles = %d\n",
            type, state, di->batt_chargecycles);
    }

    if (batt_volt >= CVT_VALID_VOLT_MV) {
        if (((batt_curr >= CVT_T1_RECOUP_CURR) &&
            (batt_curr <= CVT_T1_CURR_HIGH))) {
            if (!di->cvt_info->cnt_start) {
                di->cvt_info->cnt_start = TRUE;
                di->cvt_info->cvt_start_time = di->coul_dev_ops->get_coul_time();
                if (batt_curr < CVT_T1_CURR_LOW)
                    di->cvt_info->recoup_T1 = 1;
                coul_core_info("cvt process, T1 = %d, start_time = %d\n",
                    di->cvt_info->recoup_T1,
                    di->cvt_info->cvt_start_time);
            }
        } else if ((batt_curr >= CVT_T2_RECOUP_CURR) &&
            (batt_curr <= CVT_T2_CURR_HIGH)) {
            if (di->cvt_info->cnt_start) {
                di->cvt_info->cnt_end = TRUE;
                di->cvt_info->cvt_end_time = di->coul_dev_ops->get_coul_time();
                if (batt_curr < CVT_T2_CURR_LOW)
                    di->cvt_info->recoup_T2 = 1;
                coul_core_info("cvt process, T2 = %d, end_time = %d\n",
                    di->cvt_info->recoup_T2,
                    di->cvt_info->cvt_end_time);
            }
        } else if (batt_curr < CVT_T2_RECOUP_CURR &&
            batt_curr > CVT_T1_CURR_HIGH) {
            cvt_info_clear(di);
            coul_core_info("cvt process, batt_curr = %d\n", batt_curr);
        }
    } else {
        cvt_info_clear(di);
    }

    if (di->cvt_info->cnt_end) {
        di->cvt_info->org_cvt = (di->cvt_info->cvt_end_time -
            di->cvt_info->cvt_start_time) + (di->cvt_info->recoup_T1 -
            di->cvt_info->recoup_T2) * CVT_CURR_RECOUP_S;
        coul_core_info("org_cvt = %d, batt_cycle = %d, batt_temp = %d\n",
            di->cvt_info->org_cvt, di->batt_chargecycles, di->batt_temp);
        if ((di->batt_temp >= CVT_RECORD_TEMP_MIN) &&
            (di->batt_temp <= CVT_RECORD_TEMP_MAX)) {
            cvt_adjust(di);
            cvt_imonitor_record(di, &cvt);
            coul_imonitor_periodic_report(COUL_IMONITOR_CVT_BASIC_INFO, &cvt);
            cvt_update_hist_info(di);
            cvt_writer_to_file(di);
        } else {
            coul_core_info("cvt_process, batt temp over range\n");
        }
    }
}

/**********************************************************
*  Function:       cvt_hist_info_init
*  Description:    create cvt.data
*  Parameters:     di:smartstar_coul_device
*  return value:   void
**********************************************************/
static void cvt_hist_info_init(struct smartstar_coul_device *di)
{
    int flags;
    int read_size;
    int whence = 0;
    ...
    if (IS_ERR(fd)) {
        ...
        goto cvt_splash2_mount_file;
    }
    while ((read_size = kernel_read(fd, fd->f_pos, buff + whence,
        ...
        if (whence > (int)strlen(SPLASH2_MOUNT_INFO)) {
            (void)memmove_s(buff, MOUNTS_INFO_FILE_MAX_SIZE, buff + whence -
                strlen(SPLASH2_MOUNT_INFO), strlen(SPLASH2_MOUNT_INFO));
            (void)memset_s(buff + strlen(SPLASH2_MOUNT_INFO), whence -
                strlen(SPLASH2_MOUNT_INFO), 0, whence -
    ...
    if (!find_str) {
        ...
        goto cvt_read_proc_mounts;
    }
    filp_close(fd, NULL);
    coul_core_info("splash2 had been mounted\n");

    /* create directory /splash2/battery for data */
    coul_core_info("checking directory %s\n", CVT_DATA_DIRECTORY);
    old_fs = get_fs(); /* lint !e501 */
    set_fs(KERNEL_DS); /* lint !e501 */
    file_des = sys_access(CVT_DATA_DIRECTORY, 0);
    set_fs(old_fs);
    /* case for different errors need differenr process here */
    if (file_des < 0) {
        coul_core_info("Access directory %s failed %d in %s\n",
            CVT_DATA_DIRECTORY, file_des, __func__);
        set_fs(KERNEL_DS); /* lint !e501 */
        file_des = sys_mkdir(CVT_DATA_DIRECTORY, 0775);
        set_fs(old_fs);
        if (file_des < 0) {
            coul_core_err("Create %s for cvt failed %d in %s\n",
                CVT_DATA_DIRECTORY, file_des, __func__);
            goto cvt_init_buff_free;
        }
    }
    set_fs(KERNEL_DS); /* lint !e501 */
    cvt_dir_chown = sys_chown(CVT_DATA_DIRECTORY, ROOT_UID, SYSTEM_GID);
    set_fs(old_fs);
    if (cvt_dir_chown)
        coul_core_err("chown directory %s failed %d in %s\n",
            CVT_DATA_DIRECTORY, cvt_dir_chown, __func__);
    if (get_batt_reset_flag() || !coul_battery_cycle_count())
        batt_reset_flag = TRUE;
    memset_s(buff, sizeof(MOUNTS_INFO_FILE_MAX_SIZE + 1), 0,
        sizeof(MOUNTS_INFO_FILE_MAX_SIZE + 1));
    /* init cvt history information from /splash2/battery/cvt.data */
    flags = O_RDWR | O_CREAT | (batt_reset_flag ? O_TRUNC : 0);
    set_fs(KERNEL_DS);
    fd = filp_open(CVT_DATA_FILE, flags, 0664);
    set_fs(old_fs);
    if (IS_ERR(fd)) {
        coul_core_err("Open and create %s failed\n", CVT_DATA_FILE);
        goto cvt_init_buff_free;
    }
    read_size = kernel_read(fd, 0, buff, sizeof(di->cvt_file) + 1);
    filp_close(fd, NULL);
    if (read_size == sizeof(di->cvt_file)) {
        coul_core_info("cvt datum file size was correct\n");
        memcpy_s(&di->cvt_file, sizeof(di->cvt_file), buff, read_size);
        if ((di->cvt_file.magic_num != CVT_FILE_MAGIC_NUM) ||
            (di->cvt_file.valid_num > CVT_MAX_SAVE_NUM)) {
            coul_core_info("cvt datum file was damaged\n");
            di->cvt_file.cvt_status = 0;
            di->cvt_file.valid_num = 0;
            di->cvt_file.magic_num = CVT_FILE_MAGIC_NUM;
        }
    } else {
        coul_core_info("cvt datum file size was uncorrect\n");
    }
    di->cvt_info->cvt_retry = CVT_SPLASH2_INIT_OK;

cvt_init_buff_free:
    kfree(buff);
    buff = NULL;
    if (di->cvt_info->cvt_retry < CVT_SPLASH2_INIT_RETRY) {
        di->cvt_info->cvt_retry++;
    }
    return;
cvt_read_proc_mounts:
    filp_close(fd, NULL);
cvt_splash2_mount_file:
    kfree(buff);
    buff = NULL;
cvt_init_retry:
    if (di->cvt_info->cvt_retry < CVT_SPLASH2_INIT_RETRY) {
...
/**********************************************************
*  Function:       coul_core_get_cvt_policy
*  Discription:    get the cvt configuration parameters in dts
*  Parameters:     np: device_node
*                  cvt: coul_cvt_info
*  return value:   void
**********************************************************/
static void coul_core_get_cvt_policy(struct device_node *np,
                                     struct coul_cvt_info *cvt)
{
    int ret;
    unsigned int i;
    int array_len;
    ...
    cvt_tmp = kmalloc(sizeof(unsigned short) * CVT_RECOUP_RATIO_COUNT *
        CVT_RECOUP_RATIO_TYPE, GFP_KERNEL);
    if (!cvt_tmp) {
        coul_core_err("failed to malloc memory\n");
        return;
    }

    array_len = of_property_count_u32_elems(np, "cvt_para");
    if ((array_len <= 0) || (array_len % CVT_RECOUP_RATIO_TYPE != 0)) {
        coul_core_err("cvt_para is invaild, please check cvt_para number\n");
    } else if (array_len > (CVT_RECOUP_RATIO_TYPE * CVT_RECOUP_RATIO_COUNT)) {
        coul_core_err("cvt_para too long, use front %d paras\n", array_len);
    } else {
        ret = of_property_read_u32_array(np, "cvt_para", cvt_tmp, array_len);
        if (ret) {
            coul_core_err("dts:get cvt_para fail\n");
        } else {
            for (i = 0; i < CVT_RECOUP_RATIO_COUNT; i++) {
                cvt->recoup_temp[i] = cvt_tmp[(int)(CVT_RECOUP_TEMP +
                    CVT_RECOUP_RATIO_TYPE * i)];
                cvt->recoup_ratio[i] = cvt_tmp[(int)(CVT_RECOUP_RATIO +
                    CVT_RECOUP_RATIO_TYPE * i)];

                coul_core_info("level = %d, recoup_temp: %d, recoup_ratio: %d\n",
                    i, cvt->recoup_temp[i], cvt->recoup_ratio[i]);
            }
        }
    }
    kfree(cvt_tmp);
...
/**********************************************************
*  Function:      cvt_work
*  Discription:   the process of the CVT program,
                  start and stop with charging status
*  Parameters:    work: work_struct
*  return value:  void
**********************************************************/
static void cvt_work(struct work_struct *work)
{
    struct smartstar_coul_device *di = g_smartstar_coul_dev;

    if (di == NULL) {
        coul_core_info("cvt_work di is NULL\n");
        return;
    }

    (void)work;
    if ((di->cvt_info->cvt_retry != CVT_SPLASH2_INIT_OK) &&
        (di->cvt_info->cvt_retry < CVT_SPLASH2_INIT_RETRY)) {
        cvt_hist_info_init(di);
        coul_core_info("cvt_work, retry = %d\n", di->cvt_info->cvt_retry);
    }

    if ((di->batt_soc >= CVT_PROCESS_START_SOC) &&
        (di->charging_state == CHARGING_STATE_CHARGE_START))
        charge_batt_cvt_process(di);
    else
        coul_core_info("cvt_work, soc = %d, charging_state = %d\n",
            di->batt_soc, di->charging_state);

    if (di->cvt_info->cnt_end == TRUE)
        cvt_info_clear(di);
    else
        queue_delayed_work(system_power_efficient_wq, &di->cvt_info->cvt_delayed_work,
            round_jiffies_relative(msecs_to_jiffies(CVT_WORK_INTERVAL_MS)));
}

/**********************************************************
*  Function:      cvt_probe
*  Discription:   cvt_probe function
*  Parameters:    di: smartstar_coul_device
*  return value:  void
**********************************************************/
static void cvt_probe(struct smartstar_coul_device *di)
{
    struct device_node *np = NULL;
    struct coul_cvt_info *cvt_info = NULL;

    cvt_info = kzalloc(sizeof(struct coul_cvt_info), GFP_KERNEL);
    if (di == NULL || cvt_info == NULL) {
        coul_core_err("cvt_probe, failed to alloc cvt struct\n");
        return;
    }
    di->cvt_info = cvt_info;

    INIT_DELAYED_WORK(&di->cvt_info->cvt_delayed_work, cvt_work);
    np = di->dev->of_node;
    if (np == NULL)
        coul_core_err("cvt_probe, isc dts node can't find\n");
    else
        coul_core_get_cvt_policy(np, cvt_info);
}

/**********************************************************
*  Function:      cvt_remove
*  Discription:   cvt_remove function, called when charging stopped
*  Parameters:    di: smartstar_coul_device
*  return value:  void
**********************************************************/
static void cvt_remove(struct smartstar_coul_device *di)
{
    if (di && di->cvt_info) {
        kfree(di->cvt_info);
        di->cvt_info = NULL;
...
static void coul_charging_stop (struct smartstar_coul_device *di)
{
    ...
    if (di->cvt_support)
        cancel_delayed_work(&di->cvt_info->cvt_delayed_work);
}
...
static void coul_charging_done (struct smartstar_coul_device *di)
{
	...
	/* fast fcc drop the charge done ocv */
	di->ocv_invalid_for_fast_fcc = 1;
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    case COUL_SYSFS_GAUGELOG_HEAD:
		return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1,
			"ss_VOL  ss_CUR  ss_ufSOC  ss_SOC  SOC  ss_RM  ss_FCC  Qmax     ss_UUC  ss_CC  ss_dRC  pdFlag Temp  ss_OCV   rbatt  fcc    ocv_level  fcc_flag   ocv_old    p_vol   p_cur    err_a    polar_vol   avg_cur    polar_ocv    asw_stat  asw_vol  ");
    case COUL_SYSFS_GAUGELOG:
		...
		ret = snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1,
			"%-6d  %-6d  %-8d  %-6d  %-3d  %-5d  %-6d  %-6d  %-7d  %-5d  %-6d  %-5d  %-4d  %-7d  %-5d  %-5d  %-9d   %-8d   %-7d    %-5d    %-5d    %-5d    %-9ld    %-7d    %-9d    %-8d %-8d    ",
			voltage, (signed short)cur, ufcapacity, capacity,
			afcapacity, rm, fcc, high_pre_qmax, uuc, cc,
			delta_rc, pd_charge, temp, ocv, rbatt,
	...
	struct smartstar_coul_device *di)
{
	...
	basp_tmp = kmalloc(sizeof(unsigned int) *
...
static void coul_core_get_dts(struct smartstar_coul_device *di)
{
	...
	unsigned int fast_fcc_enable = 0;
	...
	if (of_property_read_u32(np, "fast_fcc_enable", &fast_fcc_enable))
		coul_core_err("dts:can not get fast_fcc_enable, use default : %d\n", fast_fcc_enable);
	di->fast_fcc_enable = fast_fcc_enable;
	coul_core_info("dts:get fast_fcc_enable = %d\n", fast_fcc_enable);
    ...
    if (of_property_read_u32(np, "cvt_support", &cvt_support)) {
        coul_core_err("dts:get cvt_support fail, use default limit value\n");
    }
    di->cvt_support = cvt_support;
    coul_core_info("cvt_support = %d\n",di->cvt_support);
	...
	if(of_property_read_u32(np, "basp_mask_delay", &basp_mask_delay)) {
		coul_core_err("dts:get basp_mask_delay fail, use default value!\n");
	} else {
		coul_core_info("basp_mask_delay = %d\n", basp_mask_delay);
	}
...
static ssize_t isc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    spin_lock(&iscd->boot_complete);
    iscd->app_ready = 1;
	g_smartstar_coul_dev->system_ready = 1;
