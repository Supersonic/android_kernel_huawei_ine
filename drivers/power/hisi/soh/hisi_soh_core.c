...
static void soh_acr_dsm_report(struct soh_acr_device *di)
{
#if defined CONFIG_HUAWEI_DSM
    int ret = 0;
    int batt_fcc = 0;
    char *batt_brand = NULL;
    char acr_buf[DSM_BATTERY_MAX_SIZE] = {0};
    ...
    snprintf_s(acr_buf, DSM_BATTERY_MAX_SIZE-1, DSM_BATTERY_MAX_SIZE-1, "battery: %s, batt_vol = %dmV,"
            " batt_temp = %d, full capacity = %dmAh, batt_cycle_count = %d,"
            " acr_data = %dmohm\n",
            batt_brand, di->soh_acr_info.batt_vol, di->soh_acr_info.batt_temp,
            batt_fcc, di->soh_acr_info.batt_cycle, di->soh_acr_info.batt_acr);
    //report info dmd for data collection
    ret = dsm_report(ERROR_WEAKSOURCE_HAPPEN, acr_buf);//temporary dsm err-code, later modify
    if(di->soh_acr_info.batt_acr > SOH_ACR_UPPER_LIMIT){
        //report error dmd for problem
        ret |= dsm_report(ERROR_BATT_ACR_OVER_THRESHOLD, acr_buf);
    }
    if(ret != 0)
        hisi_soh_info("[%s] acr_data dsm report error!\n", __func__);
#endif
    return;
...
static void soh_acr_calculate_work(struct work_struct *work)
{
    ...
    if (!ret) {
        ...
        soh_acr_dsm_report(di);/*report acr dmd*/
