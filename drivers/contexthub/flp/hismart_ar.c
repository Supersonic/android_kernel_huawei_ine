...
static void fifo_in(ar_data_buf_t *pdata, char *data, unsigned int len, unsigned int align)
{
	...
	if ((pdata->data_count + len) > pdata->buf_size) {
		...
		if (deta % align) {
			printk(KERN_ERR "fifo_in data not align\n");
...
static int  ar_stop_cmd(ar_port_t *ar_port, unsigned long arg)
{
	...
	if (!(FLP_AR_DATA & ar_port->channel_type) || !(FLP_AR_DATA & g_ar_dev.service_type)) {
		pr_err("[%s]had stopped[%x][%x]\n", __func__, ar_port->channel_type, g_ar_dev.service_type);
		ret =  -EIO;
		goto AR_STOP_ERR;
	}
	...
	printk(HISI_AR_DEBUG "[%s]delay[%u]\n", __func__, delay);
...
context_dev_info_t* devinfo, unsigned int context_max)
{
	...
	for(i = 0; i < devinfo->cfg.context_num; i++,plist++) {
		...
		printk(HISI_AR_DEBUG "c[%u]e[%u][%u]\n", plist->head.context,
			plist->head.event_type, plist->head.len);
	}
	return 0;
}
	...
	unsigned long arg, unsigned int context_max)
{
	...
	if (ret)
		pr_err("%s create_ar_port_cfg error\n", __func__);
	return ret;
}
...
void data_dump(void)
{
	...
	for (i = 0;i < AR_ENVIRONMENT_END;i++) {
		printk(HISI_AR_DEBUG"[%d]INITctx[0x%x]ENctx[0x%x]type[%d]\n", i, envdev_priv->env_init[i].context,
...
static int env_enable_cmd(ar_port_t *ar_port, unsigned long arg)
{
	...
	printk(HISI_AR_DEBUG"e[%u]c[%u]r[%u]l[%u]\n",enable.head.event_type, enable.head.context, enable.head.report_interval, enable.head.len);
...
static int  env_disable_cmd(ar_port_t *ar_port, unsigned long arg)
{
	...
	mutex_lock(&g_ar_dev.lock);
	if (!(FLP_ENVIRONMENT & ar_port->channel_type) || !(FLP_ENVIRONMENT & g_ar_dev.service_type)) {
		pr_err("[%s]you must first open[%x][%x]\n", __func__, ar_port->channel_type, g_ar_dev.service_type);
		ret = -EPERM;
		goto ENV_DIS_ERR;
	}
...
/*lint +e785 */
static unsigned char *context_gnetlink_alloc(ar_port_t *ar_port,unsigned int count, unsigned char cmd_type)
{
	...
	pdata->skb= genlmsg_new((size_t)count, GFP_ATOMIC);
	if (!pdata->skb) {
		pr_err("[%s]genlmsg_new error\n", __func__);
		return NULL;
	}

	pdata->msg_header = genlmsg_put(pdata->skb, 0, flp_event_seqnum++, &ar_genl_family, 0, cmd_type);
	if (!pdata->msg_header) {
		pr_err("[%s]genlmsg_put error\n", __func__);
		nlmsg_free(pdata->skb);
		return NULL;
	}

	data = nla_reserve_nohdr(pdata->skb, (int)count);
	if (!data) {
		pr_err("[%s]nla_reserve_nohdr error\n", __func__);
		nlmsg_free(pdata->skb);
		return NULL;
	}
...
/*lint -e826*/
static int context_gnetlink_send(ar_port_t *ar_port)
{
	ar_data_buf_t *pdata =  &ar_port->ar_buf;
	struct nlmsghdr *nlh;
	int result;

	nlh = (struct nlmsghdr *)((unsigned char *)pdata->msg_header - GENL_HDRLEN - NLMSG_HDRLEN);
	nlh->nlmsg_len = pdata->data_len + GENL_HDRLEN + NLMSG_HDRLEN;
	result = genlmsg_unicast(&init_net, pdata->skb, ar_port->portid);
	if (result)
		pr_err("ar:genlmsg_unicast %d", result);
...
static int ar_data_report(ar_data_buf_t *pdata, ar_port_t *ar_port)
{
	...
	data = context_gnetlink_alloc(ar_port, pdata->data_count, AR_GENL_CMD_AR_DATA);
	if (!data) {
		pr_err("[%s]context_gnetlink_alloc ERR\n", __func__);
		ret = -EBUSY;
		goto AR_DATA_REPO_FIN;
	}
	...
	if (ret) {
		nlmsg_free(pdata->skb);
		pr_err("[%s]fifo_out ERR\n", __func__);
		goto AR_DATA_REPO_FIN;
	}

	ret = context_gnetlink_send(ar_port);
AR_DATA_REPO_FIN:
	return ret;
}
...
static int get_ar_data_from_mcu(const pkt_header_t *head)
{
	...
	list_for_each(pos, &g_ar_dev.list) {
		...
		for (i = 0; i < pd->context_num; i++) {
			...
			printk(HISI_AR_DEBUG "etype[%u]ctt[%u]msec[%llu]len[%u][%u][%u][%u]\n",
			pevent->event_type, pevent->context, pevent->msec, pevent->buf_len,
			ar_port->channel_type, ar_port->flushing,port_etype);
			if((port_etype & pevent->event_type) || ((FLP_AR_DATA & ar_port->flushing) && (!pevent->event_type))) {
				if (pevent->buf_len <= CONTEXT_PRIVATE_DATA_MAX){
					...
					printk(HISI_AR_DEBUG"type[%u]ctx[%u]msec[%llu]len[%u]\n",
						pevent->event_type, pevent->context, pevent->msec, pevent->buf_len);
				}else {
...
static int get_env_data_from_mcu(const pkt_header_t *head)
{
	...
	if(NULL == ar_port) {
		pr_err("[%s]env_port must init first\n", __func__);
		goto ENV_DATA_FIN;
	}
	...
	for (i = 0; i < pd->context_num; i++) {
		...
		if(AR_ENVIRONMENT_END <= pevent->context) {
			pr_err("[%s]pevent->context[%d]idx[%d]\n", __func__, pevent->context,i);
			goto ENV_DATA_FIN;
		}

		if (pevent->buf_len > CONTEXT_PRIVATE_DATA_MAX) {
			pr_err("[%s]type[%d]len[%d] private data too large.idx[%d]\n",
				__func__,pevent->event_type,pevent->buf_len,i);
			goto ENV_DATA_FIN;
		}

		port_etype = g_ar_dev.envdev_priv.env_enable[pevent->context].head.event_type;
		printk(HISI_AR_DEBUG "num[%u]type[%u]ctt[%u]msec[%llu]len[%u][0x%x][%u]\n",
		pd->context_num, pevent->event_type, pevent->context, pevent->msec, pevent->buf_len,
		ar_port->channel_type,port_etype);
	...
	if (0 == fifo_len(pdata))
		goto ENV_DATA_FIN;
	data = context_gnetlink_alloc(ar_port, pdata->data_count, AR_GENL_CMD_ENV_DATA);
	if (!data) {
		pr_err("[%s]context_gnetlink_alloc ERR\n", __func__);
		ret = -EBUSY;
		goto ENV_DATA_FIN;
...
static int ar_state_v2_cmd(ar_port_t *ar_port, unsigned long arg)
{
	...
	ret = ar_send_cmd_from_kernel_response(TAG_AR, CMD_CMN_CONFIG_REQ,
		SUB_CMD_FLP_AR_GET_STATE_REQ, NULL, (size_t)0, &rd);
#else
	ret = ar_send_cmd_from_kernel_response(TAG_AR, CMD_CMN_CONFIG_REQ,
		CMD_FLP_AR_GET_STATE_REQ, NULL, (size_t)0, &rd);
#endif
	if (ret) {
		pr_err("[%s]ar_send_cmd_from_kernel_response err[%d]\n", __func__, rd.errno);
		goto AR_STATE_V2_ERR;
	}
...
static int  env_data_cmd(ar_port_t *ar_port, unsigned long arg)
{
	...
	if (copy_from_user((void *)&download,  (void __user *)arg, sizeof(env_data_download_t))) {
		pr_err("[%s] copy_from_user  download error \n", __func__);
		ret = -EDOM;
		goto ENV_DATA_FIN;
	}
	...
	if (copy_from_user(data,  (void __user *)download.bufaddr, download.len)) {
		pr_err("[%s] copy_from_user error [%pK]len[%u]\n", __func__, download.bufaddr, download.len);
		ret = -EDOM;
		goto ENV_DATA_ERR;
	}
#ifdef CONFIG_CONTEXTHUB_SHMEM
...
#endif
	if (ret) {
		pr_err("[%s]ar_send_cmd_from_kernel ERR[%d]\n", __func__, ret);
		ret = -EFAULT;
		goto AR_FLUSH_ERR;
	}
...
static int ar_common_ioctl_open_service(void)
{
	...
	memset_s((void*)&rd, sizeof(struct read_info), 0,sizeof(struct read_info));
	pr_info("[%s]SER[%x]ARcnt[%d]\n",__func__, g_ar_dev.service_type, g_ar_dev.ar_devinfo.usr_cnt);
	...
	if (g_ar_dev.service_type & FLP_ENVIRONMENT) {
		ret = ar_send_cmd_from_kernel_response(TAG_ENVIRONMENT, CMD_CMN_OPEN_REQ, 0, NULL, (size_t)0, &rd);
		if(ret) {
			pr_err("[%s]ar_send_cmd_from_kernel_response\n",__func__);
			return ret;
		}
...
static int ar_common_ioctl(ar_port_t *ar_port, unsigned int cmd, unsigned long arg)
{
	...
	case FLP_IOCTL_COMMON_SLEEP:
		printk(HISI_AR_DEBUG "ar:start timer %d\n",  data);
	...
	break;
	default:
		printk(KERN_ERR "ar_common_ioctl input cmd[0x%x] error\n", cmd);
		return -EFAULT;
	}
	return 0;
}
...
static long ar_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	printk(HISI_AR_DEBUG "[%s]cmd[0x%x]\n\n", __func__, cmd&0x0FFFF);
	mutex_lock(&g_ar_dev.lock);
...
static void ar_timerout_work(struct work_struct *wk)
{
	...
	pr_info("[%s]\n", __func__);
	mutex_lock(&g_ar_dev.lock);
	data = context_gnetlink_alloc(ar_port, 0, ar_port->work_para);
	if (data)
		context_gnetlink_send(ar_port);
	mutex_unlock(&g_ar_dev.lock);
}
...
static void ar_sleep_timeout(unsigned long data)
{
	...
	if (ar_port) {
		...
		if (ar_port->portid) {
			...
			pr_info("[%s]not set pid, timer out but not report\n", __func__);
		}
		if (ar_port->need_hold_wlock) {
			pr_info("wake_lock_timeout 2\n");
...
static int ar_open(struct inode *inode, struct file *filp)/*lint -e715*/
{
	...
	memset_s(ar_port, sizeof(ar_port_t), 0, sizeof(ar_port_t));
	...
	printk(HISI_AR_DEBUG "%s %d: v1.2 enter\n", __func__, __LINE__);
...
static int database_req(const pkt_header_t *head)
{
	...
	printk(HISI_AR_DEBUG"[%s][%d]\n", __func__, dbase->len);
	HISI_AR_DEBUG_DUMP_OPT(HISI_AR_DEBUG_DUMP, "dbase: ", DUMP_PREFIX_ADDRESS,
		16, 1, dbase, dbase->len, true);

	data = context_gnetlink_alloc(g_ar_dev.envdev_priv.env_port, dbase->len, AR_GENL_CMD_ENV_DATABASE);
	if (!data) {
		pr_err("[%s]context_gnetlink_alloc ERR\n", __func__);
		goto DATABASE_REQ_FIN;
	}
	...
	HISI_AR_DEBUG_DUMP_OPT(HISI_AR_DEBUG_DUMP, "data: ", DUMP_PREFIX_ADDRESS,
		16, 1, dbase, dbase->len, true);
	context_gnetlink_send(g_ar_dev.envdev_priv.env_port);
DATABASE_REQ_FIN:
	mutex_unlock(&g_ar_dev.lock);
	return 0;
} /*lint !e715*/
...
static int __init ar_init(void)
{
	...
	if (ret) {
		pr_err("[%s]genl_register_family err[%d]\n", __func__, ret);
		return ret ;
	}

	ret = misc_register(&ar_miscdev);
	if (ret){
		pr_err("cannot register hisi ar err=%d\n", ret);
		goto AR_PROBE_GEN;
	}
