...
#define FLP_CELLBATCHING (BIT(6))

#ifdef CONFIG_HISI_GEOFENCE_TRAJECTORY_FEATURE
#define IOMCU_APP_GNSS      (FLP_BATCHING|FLP_GEOFENCE)
#endif
#define IOMCU_APP_FLP       (FLP_BATCHING|FLP_GEOFENCE|FLP_CELLFENCE|FLP_CELLTRAJECTORY|FLP_WIFIFENCE|FLP_CELLBATCHING)
...
static void flp_resort_cellbatching(char * data, unsigned int len)
{
    ...
    list_for_each(pos, &g_flp_dev.list) {
        ...
        switch (((pkt_subcmd_req_t *)head)->subcmd)  {
            ...
            case SUB_CMD_FLP_CELL_CELLBATCHING_REPORT_REQ:
                flp_resort_cellbatching(data, len);
            ...
            case SUB_CMD_FLP_WIFENCE_STATUS_REQ:
                ...
                break;
            case SUB_CMD_FLP_WIFIDB_LOCATION_REPORT_REQ:
                flp_send_data_to_uplayer(flp_port, data, len, FLP_OFFLINEDB, FLP_GENL_CMD_REQUEST_WIFIDB);
                break;
            default:
...
#else
	printk(KERN_ERR "%s 1.1 not support cmd[%d]\n", __func__, cmd);
	ret = -EFAULT;
#endif
	if (ret) {
		pr_err("[%s]iomcu err[%d]\n", __func__, rd.errno);
...
#else
    printk(KERN_ERR "%s 1.1 not support cmd[%ld]\n", __func__, arg);
#endif
    return 0;
}
...
static int get_cellbatching_cfg(celltrajectory_start_config_t *ct_cfg, const char __user *buf)
{
	if (copy_from_user(ct_cfg, buf, sizeof(celltrajectory_start_config_t))) {
		printk(KERN_ERR "get_celltrajectory_cfg copy_from_user error\n");
		return -EIO;
	}
	return 0;
}

static int cellbatching_cfg(flp_port_t *flp_port, unsigned long arg)
{
	int ret;

       celltrajectory_start_config_t ct_cfg;
       if (get_cellbatching_cfg(&ct_cfg, (char __user * )arg))
		return -EFAULT;
	...
	ret = send_cmd_from_kernel(TAG_FLP, CMD_CMN_CONFIG_REQ,
		SUB_CMD_FLP_CELL_CELLBATCHING_CFG_REQ, (char *)&ct_cfg, sizeof(celltrajectory_start_config_t));
...
static int cellbatching_request(flp_port_t *flp_port)
{
...
static int cellfence_ioctl(flp_port_t *flp_port, unsigned int cmd, unsigned long arg)
{
	...
	case FD_IOCTL_CELLBATCHING_CONFIG:
		return cellbatching_cfg(flp_port, arg);
	case FD_IOCTL_CELLBATCHING_REQUEST:
		return cellbatching_request(flp_port);
	default:
		pr_err("[%s]cmd err[%u] \n", __func__, cmd);
		return -EINVAL;
	}
...
static int flp_oper_wififence(flp_port_t *flp_port, unsigned long arg, unsigned int cmd)
{
    ...
    cmd_data = (char *)kzalloc((size_t)config.length, GFP_KERNEL);
    if (!cmd_data) {
        printk(KERN_ERR "%s kmalloc fail\n", __func__);
        return -ENOMEM;
    }
...
static int flp_open(struct inode *inode, struct file *filp)/*lint -e715*/
{
	...
	printk(HISI_FLP_DEBUG "%s %d: v1.2 enter\n", __func__, __LINE__);
	return 0;
FLP_OPEN_ERR:
	mutex_unlock(&g_flp_dev.lock);
	return ret;
...
static int flp_mmap(struct file *filp, struct vm_area_struct *vma)
{
    ...
    list_for_each(pos, &g_flp_dev.list) {
        flp_port = container_of(pos, flp_port_t, list);
        if (flp_port->has_maped) {
            printk(KERN_ERR "[%s]:line[%d] port has maped, map does not support multi\n", __func__, __LINE__);
            mutex_unlock(&g_flp_dev.lock);
            return -1;
        }
    }
    ...
    printk(HISI_FLP_DEBUG "[%s]:line[%d] enter, vm_start.0x%lx, size.0x%lx, vm_end0x%lx\n",
         __func__, __LINE__, vma->vm_start, size, vma->vm_end);
