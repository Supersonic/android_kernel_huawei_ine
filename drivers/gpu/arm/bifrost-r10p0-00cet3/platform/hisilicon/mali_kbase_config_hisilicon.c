...
static inline void kbase_platform_on(struct kbase_device *kbdev)
{
	if (kbdev->regulator) {
		...
		if (kbdev->gpu_vid == 0) {
			kbdev->gpu_vid = kbase_reg_read(kbdev, GPU_CONTROL_REG(GPU_ID), NULL);
...
static void pm_callback_power_off(struct kbase_device *kbdev)
{
	...
	if (kbase_has_hi_feature(kbdev, KBASE_FEATURE_HI0008)) {
		...
		kbase_reg_write(kbdev, GPU_CONTROL_REG(TILER_PWROFF_LO), tiler_lo, NULL);
		kbase_reg_write(kbdev, GPU_CONTROL_REG(TILER_PWROFF_HI), tiler_hi, NULL);
		kbase_reg_write(kbdev, GPU_CONTROL_REG(L2_PWROFF_LO), l2_lo, NULL);
		kbase_reg_write(kbdev, GPU_CONTROL_REG(L2_PWROFF_HI), l2_hi, NULL);
	}
