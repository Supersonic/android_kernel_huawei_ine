...
int kbase_gpu_mmap(struct kbase_context *kctx, struct kbase_va_region *reg, u64 addr, size_t nr_pages, size_t align, bool set_scramble_bit)
{
	int err;
	size_t i = 0;
	unsigned long attr;
	...
	if (reg->gpu_alloc->type == KBASE_MEM_TYPE_ALIAS) {
		...
		if (set_scramble_bit && (reg->flags & KBASE_REG_SCRAMBLE_BIT)) {
			reg->flags &= ~KBASE_REG_SCRAMBLE_BIT;
			/* Header pages is calculated from buffer_size = header_size(width*height/16) + payload_size(width*height*4)
			add one page for header to solve aligned issue*/
			err = kbase_mmu_insert_pages_with_scramble_bit(kctx, reg->start_pfn,
					kbase_get_gpu_phy_pages(reg),
					kbase_reg_current_backed_size(reg),
					reg->flags & gwt_mask, (int)(nr_pages/64)+1);
		}else {
