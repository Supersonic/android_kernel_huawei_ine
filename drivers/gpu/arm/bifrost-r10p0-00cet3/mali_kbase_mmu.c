...
static void page_fault_failed_memory_free(struct kbase_va_region *region,
				struct record_allocated_pages *rd_al_pages, int loop)
{
	int i;

	for (i = 0; i < loop; i++) {
		struct kbase_mem_pool *pool = rd_al_pages[i].pool;

		if (!pool)
			return;

		kbase_mem_pool_lock(pool);
		kbase_free_phy_pages_helper_locked(region->gpu_alloc,
				pool,rd_al_pages[i].gpu_pages,
				rd_al_pages[i].nr_pages);
		if (region->gpu_alloc != region->cpu_alloc) {
			kbase_free_phy_pages_helper_locked(region->cpu_alloc,
					pool,rd_al_pages[i].cpu_pages,
					rd_al_pages[i].nr_pages);
		}
...
void page_fault_worker(struct work_struct *data)
{
	u64 fault_pfn;
	u32 fault_status;
	size_t new_pages, tmp_new_pages, pages_to_alloc;
	...
	struct kbase_va_region *region;
	int err;
	bool grown = false, large_pool=false;
	size_t min_pool_size, total_pool_size = 0;
	struct kbase_mem_pool *pool, *tmp_pool, *locked_pool[POOL_LEVEL];
	int pages_to_grow;
	struct tagged_addr *gpu_pages = NULL, *cpu_pages=NULL;
	struct record_allocated_pages rd_al_pages[POOL_LEVEL];
	int loop = 0;
...
page_fault_retry:
	...
	memset(rd_al_pages, 0x0, sizeof(rd_al_pages));
	memset(locked_pool, 0x0, sizeof(locked_pool));
	total_pool_size = 0;
	...
	/* culculate if three pool size is fit all the memory needed */
	for (loop = 0; loop < POOL_LEVEL; loop++) {
		if (tmp_pool) {
			/* Need lock pool till memory allocated finish or pool size may change*/
			kbase_mem_pool_lock(tmp_pool);
			locked_pool[loop] = tmp_pool;
			total_pool_size += kbase_mem_pool_size(tmp_pool);
			tmp_pool = tmp_pool->next_pool;
		} else {
			break;
		}
	}

	if (total_pool_size  >= min_pool_size) {
		loop = 0;
		pages_to_alloc = new_pages;
		tmp_pool = pool;

		do {
			if (large_pool) {
				/* Round up to number of 2 MB pages required */
				min_pool_size = pages_to_alloc + ((SZ_2M / SZ_4K) - 1);
				min_pool_size /= (SZ_2M / SZ_4K);
			} else {
				min_pool_size = pages_to_alloc;
			}
			 ...
			 */
			if (kbase_mem_pool_size(tmp_pool) >= min_pool_size) {
				gpu_pages = kbase_alloc_phy_pages_helper_locked(
						region->gpu_alloc, tmp_pool, pages_to_alloc);

				if (gpu_pages) {
					if (region->gpu_alloc != region->cpu_alloc) {
						cpu_pages = kbase_alloc_phy_pages_helper_locked(
								region->cpu_alloc, tmp_pool,
								pages_to_alloc);
				...
				/*record allocated pages, if failed need to free */
				if (grown) {
					if (region->gpu_alloc != region->cpu_alloc)
						rd_al_pages[loop].cpu_pages = cpu_pages;

					rd_al_pages[loop].gpu_pages = gpu_pages;
					rd_al_pages[loop].pool = tmp_pool;
					rd_al_pages[loop].nr_pages = pages_to_alloc;
				}

				/* clear allocate page to avoid infinit loop*/
				pages_to_alloc = 0;
			} else {
				/* allocate the part size from one pool, others from another pool */
				if (large_pool) {
					/* Caulculate the pages size of 2M alloc */
					tmp_new_pages = kbase_mem_pool_size(tmp_pool) * (SZ_2M / SZ_4K);
				} else {
					tmp_new_pages = kbase_mem_pool_size(tmp_pool);
				}

				if (region->gpu_alloc != region->cpu_alloc) {
					tmp_new_pages = tmp_new_pages/2;
				}

				/* only one page left in pool, if didn't handle specially will cause infinite loop */
				if (0 != tmp_new_pages) {
					gpu_pages = kbase_alloc_phy_pages_helper_locked(
							region->gpu_alloc, tmp_pool, tmp_new_pages);
					if (gpu_pages) {
						if (region->gpu_alloc != region->cpu_alloc) {
							cpu_pages = kbase_alloc_phy_pages_helper_locked(
									region->cpu_alloc, tmp_pool,
						...
						pages_to_alloc = pages_to_alloc - tmp_new_pages;
						KBASE_DEBUG_ASSERT(pages_to_alloc>0);
					}

					/* whetever allocated failed break out */
					if (!gpu_pages || (!cpu_pages && (region->gpu_alloc != region->cpu_alloc))) {
						pages_to_alloc = 0;
					} else {
						/*record allocated pages, if failed need to free */
						rd_al_pages[loop].cpu_pages = cpu_pages;
						rd_al_pages[loop].gpu_pages = gpu_pages;
						rd_al_pages[loop].pool = tmp_pool;
						rd_al_pages[loop].nr_pages = tmp_new_pages;
					}
				}
			}
			tmp_pool = tmp_pool->next_pool;
		} while((loop++ < POOL_LEVEL) && !grown && pages_to_alloc && tmp_pool);
	} else {
		pages_to_grow = min_pool_size - kbase_mem_pool_size(pool);
	}

	/* unlock used pool */
	for (loop = POOL_LEVEL-1; loop >= 0; loop--){
		if(locked_pool[loop])
			kbase_mem_pool_unlock(locked_pool[loop]);
	}
	...
	if (grown) {
		...
		if (err) {
			mutex_lock(&kctx->mem_partials_lock);
			page_fault_failed_memory_free(region, rd_al_pages, sizeof(rd_al_pages));
			mutex_unlock(&kctx->mem_partials_lock);
		...
		if (ret < 0) {
			/* free previously allocated memory grow pool and retry again */
			mutex_lock(&kctx->mem_partials_lock);
			page_fault_failed_memory_free(region, rd_al_pages, sizeof(rd_al_pages));
			mutex_unlock(&kctx->mem_partials_lock);
